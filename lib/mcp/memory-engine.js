...
const VOLATILE_FACT_CATEGORIES = new Set(['work', 'goals', 'other']);
const VOLATILE_FACT_STALE_DAYS = 60;
const DEFAULT_FACT_STALE_DAYS = 180;

function factAgeDays(mem) {
  const ts = mem.last_verified_at || mem.updated_at || mem.created_at;
  if (!ts) return null;
  return (Date.now() - new Date(ts).getTime()) / 86400000;
}

// Deterministic staleness annotation — no model calls. Stale facts travel into the
// injection with their uncertainty attached instead of being silently dropped
// (or silently trusted).
export function stalenessNote(mem) {
  const days = factAgeDays(mem);
  if (mem.review_state === 'needs_review') {
    return days == null
      ? ' (unverified — confirm before relying on it)'
      : ` (unverified — last recorded ${Math.round(days)} days ago; confirm before relying on it)`;
  }
  if (days == null) return '';
  const limit = VOLATILE_FACT_CATEGORIES.has(mem.category) ? VOLATILE_FACT_STALE_DAYS : DEFAULT_FACT_STALE_DAYS;
  return days > limit
    ? ` (recorded ${Math.round(days)} days ago — may be outdated; confirm if it matters)`
    : '';
}
  ...
  if (matches && matches.length) {
    const facts = matches
      .map((m) => {
        ...
        return mem.content + stalenessNote(mem);
      })
      .filter(Boolean)
      .join('. ');
