...
import { extractMemoriesFromMessage } from '@/lib/handlers/memory-system';

const MEMORY_CATEGORIES = ['health', 'preferences', 'personal', 'work', 'relationships', 'goals', 'other'];

// Exact-match dedup helper — mirrors the extension's save path (saveExtractedMemories)
// so MCP writes never double-save a fact the extension already captured. Exact (not
// prefix) matching avoids over-skipping distinct facts like different children's names.
const escapeRegex = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

// Negative/missing-info statements the LLM should never treat as a saveable memory.
...
const isNegativeStatement = (content) => NEGATIVE_PATTERNS.some((p) => p.test(content));

// Flag-and-confirm directive appended to framed context so agents ask before saving.
const SAVING_INSTRUCTION =
  '\n\nMemory saving: if you learn a durable, non-ephemeral fact about the user ' +
  '(preferences, relationships, goals, life details, work) that is not already ' +
  'reflected above, ask "Want me to save this to SoulPrint?" and only call ' +
  'soulprint_add_memory if they confirm. Do not save transient chat details or ' +
  'facts already captured above.';
...
export async function getContext(db, user, query = '') {
  ...
  return {
    context: framed ? framed + SAVING_INSTRUCTION : null,
    profile,
...
export async function addMemory(db, user, { content, category, importance }) {
  ...
  // Dedupe: if an identical (case-insensitive) memory already exists — e.g. the
  // extension already auto-extracted it — skip the write and report it instead.
  const existing = await db.collection('user_memories').findOne({
    user_id: user.id,
    content: { $regex: `^${escapeRegex(trimmed)}$`, $options: 'i' },
  });
  if (existing) {
    return { success: true, duplicate: true, memory: existing };
  }
...
export async function suggestMemories(db, user, { user_message, assistant_message }) {
  const userMsg = (user_message || '').trim();
  const assistantMsg = (assistant_message || '').trim();
  if (!userMsg) {
    throw new Error('user_message is required');
  }

  // Run the SAME extraction engine the extension uses (gpt-4o-mini), then filter
  // negatives and dedupe against the store — so suggestions are net-new only.
  const extracted = await extractMemoriesFromMessage(userMsg, assistantMsg, user.id);

  const existingDocs = await db.collection('user_memories')
    .find({ user_id: user.id }, { projection: { content: 1 } })
    .toArray();
  const existingContents = existingDocs.map((d) => (d.content || '').toLowerCase().trim());
  ...
  for (const mem of extracted) {
    ...
    if (existingContents.includes(content.toLowerCase())) {
      duplicateCount += 1;
      continue;
    }
    candidates.push({
  ...
  return {
    ...
    total_existing: existingContents.length,
  };
}
...
export async function handleMcpTool(db, user, name, args = {}) {
  switch (name) {
    ...
    case 'soulprint_suggest_memories':
      return suggestMemories(db, user, args);
