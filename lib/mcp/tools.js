...
export const TOOLS = [
  ...
  },
  {
    ...
    description:
      'Save a new memory about the user. Confirm with the user before calling this — ask "Want me to save this to SoulPrint?" rather than saving silently.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'soulprint_suggest_memories',
    description:
      'Mine a recent user/assistant exchange for durable facts about the user worth saving to SoulPrint. Runs the same extraction engine as the browser extension and returns DEDUPLICATED candidates (facts already saved are excluded). Nothing is saved — show the candidates to the user, let them pick, then call soulprint_add_memory for each one they confirm.',
    inputSchema: {
      type: 'object',
      properties: {
        user_message: { type: 'string', description: "The user's message(s) to mine for durable facts" },
        assistant_message: { type: 'string', description: 'The assistant reply (optional context for extraction)' },
      },
