...
export function getOAuth(request) {
  ...
  return {
    ...
    // Hosts accepted for `client_id` under CIMD. ChatGPT AND Claude both present
    // their metadata document URL as the client_id (e.g.
    // https://chatgpt.com/oauth/client.json, https://claude.ai/mcp/oauth/...).
    // Claude selects CIMD whenever the discovery doc advertises
    // `client_id_metadata_document_supported: true` + `none` (which we do), so
    // claude.ai MUST be here or Claude's authorize is rejected as invalid_client
    // before the login page ever renders. Note: baseHost is deliberately excluded
    // — our own domain is not a valid OAuth client.
    clientHosts: [
      'chatgpt.com',
      'connector.chatgpt.com',
      'chat.openai.com',
