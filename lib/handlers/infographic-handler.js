...
export async function handleInfographicChat(userPrompt, send) {
  try {
    ...
    // Always save locally first as a guaranteed fallback
    const fs = await import('fs/promises');
    const localFileName = `infographic_${template}_${Date.now()}.png`;
    const localPath = `/tmp/${localFileName}`;
    await fs.writeFile(localPath, imageBuffer);
    const localFallbackUrl = `/api/infographic/serve?file=${encodeURIComponent(localPath)}`;
      ...
      try {
        ...
        if (upRes.ok) {
          ...
          if (d.success && d.data?.downloadUrl) {
            // Verify the URL is actually accessible before using it
            try {
              const checkRes = await fetch(d.data.downloadUrl, { method: 'HEAD', signal: AbortSignal.timeout(5000) });
              if (checkRes.ok) {
                imageUrl = d.data.downloadUrl;
              } else {
                console.warn(`[Infographic] Upload succeeded but URL check failed (HTTP ${checkRes.status}), using local fallback`);
              }
            } catch (checkErr) {
              console.warn(`[Infographic] Upload succeeded but URL unreachable: ${checkErr.message}, using local fallback`);
            }
          }
        }
      } catch (uploadErr) {
        console.warn(`[Infographic] Upload failed: ${uploadErr.message}, using local fallback`);
      }
    }

    // Use local fallback if upload failed or URL is not accessible
    if (!imageUrl) {
      imageUrl = localFallbackUrl;
      console.log(`[Infographic] Using local serve fallback: ${imageUrl}`);
    }
