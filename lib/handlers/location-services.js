...
function parseLocationQuery(text) {
  // Guard: if the text is too long, it's not a location query
  ...
  if (nearMatch) {
    ...
    // Reject if the extracted location is too long (real locations are short)
    if (loc.length > 100) return null;
    return loc;
  }
