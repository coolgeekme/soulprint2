...
// Derive the base URL from the incoming request's Host header so the
// verification_uri always points back to the SAME domain the client called.
// NEXT_PUBLIC_BASE_URL can carry the preview domain even on prod (see lib/email.js
// for the same guard), which would send users to the wrong /activate page and
// break device logins. Host-derived URL has no preview/production drift.
...
export async function handleDeviceAuth(request) {
  try {
    ...
    return ok({
      ...
      verification_uri: `${getBaseUrl(request)}/activate`,
