...
async function generatePersonaFromAssessment(db, userId) {
  // ── 1. Direct slider answers — have numeric values (0-100) ──
  const sliderAnswers = await db.collection('assessment_slider_answers')
    .find({ user_id: userId }).toArray();

  // ── 2. Full 36-question assessment — TEXT answers with UUID keys ──
  ...
  // Build a set of slider IDs that are "answered" from every source
  const answeredSliderIds = new Set(sliderAnswers.map(a => a.question_id));
  
  // Map full assessment UUIDs → slider IDs via order_index
  const fullAnswerPillarMap = {}; // slider_id → pillar
  if (fullAnswers.length > 0) {
    ...
    for (const ans of fullAnswers) {
      ...
      if (orderIndex) {
        const sliderId = SLIDER_QUESTIONS[orderIndex - 1]?.id;
        if (sliderId) {
          answeredSliderIds.add(sliderId);
          fullAnswerPillarMap[sliderId] = SLIDER_QUESTIONS[orderIndex - 1].pillar;
        }
      }
    }
  }

  // Count layered assessment answers toward pillar coverage
  const layeredPillars = new Set();
  if (layeredDoc) {
    const l1 = layeredDoc.layer1_answers || {};
    for (const qId of Object.keys(l1)) {
      if (qId.startsWith('comm_')) layeredPillars.add('communication');
      else if (qId.startsWith('emotion_')) layeredPillars.add('emotional_intelligence');
      else if (qId.startsWith('decision_')) layeredPillars.add('decision_making');
    }
  }

  // Group answers by pillar — use slider numeric values where available
  const pillarScores = {};
  const pillarCounts = {};

  // Process direct slider answers (these have actual numeric values)
  for (const answer of sliderAnswers) {
    ...
    if (!pillar) continue;
    if (!pillarScores[pillar]) { pillarScores[pillar] = 0; pillarCounts[pillar] = 0; }
    pillarScores[pillar] += (answer.value || 50);
    pillarCounts[pillar]++;
  }

  // For full text assessment answers, count the covered pillars.
  // Since text answers don't have numeric values, we contribute a neutral
  // score (50) for each pillar where the user answered but has NO slider data.
  for (const [sliderId, pillar] of Object.entries(fullAnswerPillarMap)) {
    // Only contribute if we don't already have a slider answer for this question
    const hasSliderAnswer = sliderAnswers.some(a => a.question_id === sliderId);
    if (!hasSliderAnswer) {
      if (!pillarScores[pillar]) { pillarScores[pillar] = 0; pillarCounts[pillar] = 0; }
      pillarScores[pillar] += 50; // Neutral baseline for text-only answers
      pillarCounts[pillar]++;
    }
  }

  // For layered assessment pillars with no other data, add neutral coverage
  for (const pillar of layeredPillars) {
    if (!pillarScores[pillar]) {
      pillarScores[pillar] = 50;
  ...
  // Total answered count from all sources (deduplicated via slider IDs)
  const totalAnswered = answeredSliderIds.size;

  console.log(`[PersonaDNA] Assessment sources: ${sliderAnswers.length} slider, ${fullAnswers.length} full-text, ${layeredPillars.size} layered pillars → ${totalAnswered} total unique answers, ${Object.keys(pillarAverages).length} pillars covered`);

  return {
    axes,
    pillarAverages,
    answeredCount: totalAnswered,
    sliderCount: sliderAnswers.length,
    source: 'assessment',
  };
}
...
async function getOrCreatePersonaProfile(db, userId) {
  ...
  // Determine if we should supplement with history mining.
  // Even if answeredCount is high, if all answers are text-based (neutral 50 scores),
  // we still benefit from history mining for directional personality data.
  const needsHistorySupport = profile.answeredCount < 8 || profile.sliderCount === 0;

  if (needsHistorySupport) {
    const historyProfile = await minePersonaFromHistory(db, userId);
    if (historyProfile) {
      // Blend: assessment data takes priority where it exists with real values
      const blended = { ...DEFAULT_AXES };
      const assessmentWeight = profile.sliderCount > 0 ? 0.7 : 0.3;
      const historyWeight = 1 - assessmentWeight;

      for (const axis of Object.keys(DEFAULT_AXES)) {
        if (profile.answeredCount > 0 && profile.axes[axis] !== DEFAULT_AXES[axis]) {
          // Assessment data exists for this axis — blend with weights
          blended[axis] = Math.round(profile.axes[axis] * assessmentWeight + historyProfile.axes[axis] * historyWeight);
        } else {
      ...
      profile = {
        ...
        sliderCount: profile.sliderCount,
...
async function handleGetPersonaProfile(request) {
  ...
  // Admins can query other users' profiles via ?userId=xxx
  let targetUserId = user.id;
  const queryUserId = url.searchParams.get('userId');
  if (queryUserId && (user.role === 'admin' || user.role === 'superadmin')) {
    targetUserId = queryUserId;
  }

  const profile = await getOrCreatePersonaProfile(db, targetUserId);
  const prompt = buildPersonaSystemPrompt(profile);
  
  // Also fetch any saved overrides
  const overrideDoc = await db.collection('persona_overrides').findOne({ user_id: targetUserId });
...
// ═══════════════════════════════════════════════════════════════════
// OVERRIDE HANDLER: Let users manually tune their personality dials
// ═══════════════════════════════════════════════════════════════════

async function handleSavePersonaOverride(request) {
  const user = await authenticate(request).catch(() => null);
  if (!user) {
    return { error: 'Unauthorized', status: 401 };
  }
  ...
  if (!overrides || typeof overrides !== 'object') {
    return { error: 'overrides object required', status: 400 };
  }

  // Validate override values (0-100 range)
  const validDials = ['directness', 'warmth', 'playfulness', 'challenge'];
  const cleanOverrides = {};
  for (const [key, val] of Object.entries(overrides)) {
    if (validDials.includes(key) && typeof val === 'number' && val >= 0 && val <= 100) {
      cleanOverrides[key] = Math.round(val);
    }
  }

  // Save overrides
  await db.collection('persona_overrides').updateOne(
    { user_id: user.id },
    {
      $set: {
        user_id: user.id,
  ...
  // Invalidate the cached persona profile so the AI picks up changes
  await invalidatePersonaProfile(db, user.id);

  console.log(`[PersonaDNA] User ${user.id.substring(0, 8)}... saved overrides:`, cleanOverrides);

  return { success: true, overrides: cleanOverrides };
}

// ═══════════════════════════════════════════════════════════════════
// CACHE INVALIDATION: Called when new assessment data comes in
// ═══════════════════════════════════════════════════════════════════

async function invalidatePersonaProfile(db, userId) {
  try {
    await db.collection('persona_profiles').deleteOne({ user_id: userId });
    console.log(`[PersonaDNA] Invalidated cached profile for ${userId.substring(0, 8)}...`);
  } catch (e) {
    console.error('[PersonaDNA] Failed to invalidate profile:', e.message);
  }
}

// ═══════════════════════════════════════════════════════════════════
// QUICK ACCESSOR: Get persona prompt string for a user (used by chat-stream)
// ═══════════════════════════════════════════════════════════════════

async function getPersonaPromptForUser(db, userId) {
  try {
    const profile = await getOrCreatePersonaProfile(db, userId);
    
    // Apply user's manual overrides (from the 4-dial UI)
    const overrideDoc = await db.collection('persona_overrides').findOne({ user_id: userId });
    if (overrideDoc?.overrides && Object.keys(overrideDoc.overrides).length > 0) {
      const ov = overrideDoc.overrides;
      // Map the 4 simplified dials back to the 10 axes
      if (ov.directness !== undefined) {
        profile.axes.directness = Math.round(ov.directness * 0.7 + profile.axes.directness * 0.3);
        profile.axes.formality = Math.round((100 - ov.directness) * 0.7 + profile.axes.formality * 0.3);
      }
      if (ov.warmth !== undefined) {
        profile.axes.warmth = Math.round(ov.warmth * 0.6 + profile.axes.warmth * 0.4);
        profile.axes.emotionalDepth = Math.round(ov.warmth * 0.5 + profile.axes.emotionalDepth * 0.5);
        profile.axes.expressiveness = Math.round(ov.warmth * 0.5 + profile.axes.expressiveness * 0.5);
      }
      if (ov.playfulness !== undefined) {
        profile.axes.humor = Math.round(ov.playfulness * 0.7 + profile.axes.humor * 0.3);
        profile.axes.pace = Math.round(ov.playfulness * 0.5 + profile.axes.pace * 0.5);
      }
      if (ov.challenge !== undefined) {
        profile.axes.challenge = Math.round(ov.challenge * 0.7 + profile.axes.challenge * 0.3);
        profile.axes.autonomy = Math.round(ov.challenge * 0.5 + profile.axes.autonomy * 0.5);
      }
      console.log(`[PersonaDNA] Applied manual overrides for ${userId.substring(0, 8)}...`);
    }
    
    return buildPersonaSystemPrompt(profile);
  } catch (e) {
    console.error('[PersonaDNA] Failed to generate persona prompt:', e.message);
    return ''; // Graceful fallback — no persona, just use default
  }
}

// ═══════════════════════════════════════════════════════════════════
// NOTIFICATION STATUS: Check if user has an unseen persona profile
// ═══════════════════════════════════════════════════════════════════

async function handleGetPersonaNotification(request) {
  const user = await authenticate(request).catch(() => null);
  if (!user) return { error: 'Unauthorized', status: 401 };

  const db = await getDb();
  
  // Check if user has a persona profile that was generated from their history
  const profile = await db.collection('persona_profiles').findOne({ user_id: user.id });
  if (!profile || (profile.source !== 'history_mining' && profile.source !== 'blended')) {
    return { hasNotification: false };
  }

  // Check if user has already seen the personality tab
  const seen = await db.collection('persona_tab_seen').findOne({ user_id: user.id });
  return { hasNotification: !seen, source: profile.source, messagesMined: profile.messagesMined || 0 };
}

async function handleMarkPersonaSeen(request) {
  const user = await authenticate(request).catch(() => null);
  if (!user) return { error: 'Unauthorized', status: 401 };

  const db = await getDb();
  await db.collection('persona_tab_seen').updateOne(
    { user_id: user.id },
    { $set: { user_id: user.id, seen_at: new Date() } },
    { upsert: true }
  );
  return { success: true };
}
...
// ═══════════════════════════════════════════════════════════════════
// FORCE REFRESH: Regenerate persona profile from scratch
// ═══════════════════════════════════════════════════════════════════

async function handleForceRefreshPersona(request) {
  const user = await authenticate(request).catch(() => null);
  if (!user) return { error: 'Unauthorized', status: 401 };

  const db = await getDb();
  
  // Delete cached profile to force regeneration
  await db.collection('persona_profiles').deleteOne({ user_id: user.id });
  console.log(`[PersonaDNA] Force refresh requested by ${user.id.substring(0, 8)}...`);
  ...
  const overrideDoc = await db.collection('persona_overrides').findOne({ user_id: user.id });
