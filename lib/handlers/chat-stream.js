        ...
        if (enableWebSearch && attachments.length === 0) {
          ...
          if (userText) {
            ...
            const isPersonalOrMemoryQuery = (
              // Personal pronouns indicating user is asking about THEMSELVES
              /\b(my|mine|i have|i am|i'm|i was|i've|do i|am i|did i|have i|what's my|whats my|how many (?:kids|children|siblings|pets|dogs|cats|friends|family).*(?:do i|i have))\b/i.test(lowerText) ||
              // Single-word or short conversational corrections
              /^(incorrect|wrong|no|nope|that's wrong|thats wrong|not right|not correct|that's incorrect|thats incorrect|false|inaccurate)\b/i.test(lowerText) ||
              // Direct personal questions
              /^(who am i|what am i|where am i from|when was i born|how old am i|what do i|what did i|where do i live|what's my name|whats my name)\b/i.test(lowerText) ||
              // Questions explicitly about user's stored data
              /\b(in my profile|from my memory|what do you know about me|tell me about my|remind me about my|according to my)\b/i.test(lowerText)
            );
