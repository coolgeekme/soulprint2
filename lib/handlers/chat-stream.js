...
async function handleChatStream(request) {
  ...
  try {
    ...
    console.log(`[Imprint] Loading for user ${user.id.substring(0, 8)}..., projectId: ${imprintProjectId || 'none'}, convProjectId: ${conv?.project_id || 'none'}`);
    const activeImprint = await getActiveImprint(user.id, imprintProjectId);
