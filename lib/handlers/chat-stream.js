...
- Output ONLY the refined prompt text, nothing else`,
        ...
        const hasVideoToExtend = lastVideoTaskIdInConversation || mediaFlow?.sourceVideoTaskId;
        ...
        if ((isVideoExtendRequest && hasVideoToExtend) || (mediaFlow?.type === 'video_extend')) {
          ...
          const sourceVideoUrl = mediaFlow?.sourceVideoUrl || lastVideoUrlInConversation;
...
- Output ONLY the refined prompt text, nothing else`,
          ...
          console.log(`[VideoExtend] Extending video task ${extendTaskId} with prompt: "${extendPrompt?.substring(0, 120)}"`);
