...
--- END PERSONA INTRODUCTION ---`;
  ...
  const detectDocumentIntent = (text) => {
    ...
    const excelKeywords = /(?:create|make|generate|build|export|download|give\s+me|can\s+you\s+make)\s+(?:an?\s+)?(?:excel|spreadsheet|xlsx|.xlsx|csv|.csv|table|data\s+file)/i;
    const spreadsheetContext = /(?:put|organize|structure|format|export|save).*(?:data|table|rows|columns).*(?:excel|spreadsheet|csv|file)/i;
    
    if (excelKeywords.test(text) || spreadsheetContext.test(text)) {
      console.log('[DocumentIntent] Detected Excel/CSV generation request');
      return { type: 'excel', format: lower.includes('csv') ? 'csv' : 'xlsx' };
    }
    
    // Word document detection
    const wordKeywords = /(?:create|make|generate|write|export|download)\s+(?:a\s+)?(?:word\s+doc|document|docx|.docx|word\s+file)/i;
    ...
    const pptKeywords = /(?:create|make|generate|build)\s+(?:a\s+)?(?:powerpoint|presentation|slides|pptx|.pptx)/i;
    ...
    const docStream = new ReadableStream({
      async start(controller) {
        try {
          ...
          const fileBlob = await docGenResponse.blob();
          
          // TODO: Upload to storage and get permanent URL
          // For now, we'll create a data URL (temporary solution)
            ...
            reader.onloadend = () => {
              ...
              controller.enqueue(encoder.encode(JSON.stringify({ 
                ...
                content: `\n\n✅ **${documentIntent.type.toUpperCase()} file generated successfully!**\n\n` +
                         ...
                         `The file is ready to download. Click the link above or use the download button.`
              }) + '\n'));
