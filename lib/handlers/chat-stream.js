...
- Output ONLY the refined prompt text, nothing else`,
          ...
          if (!extendTaskId) {
            console.log(`[VideoExtend] No taskId available — using image-to-video with frame extraction`);
            send({ type: 'delta', content: '🎬 Extracting a frame from your video and generating a continuation...\n\n' });
            fullContent = '🎬 Extracting a frame from your video and generating a continuation...\n\n';
            ...
            try {
              ...
              // Try to get a frame from the uploaded video attachment
              let seedImageUrl = null;
              
              // Check if any attachment has base64 video data we can extract a frame from
              if (attachments && attachments.length > 0) {
                for (const att of attachments) {
