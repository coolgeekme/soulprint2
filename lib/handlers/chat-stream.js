...
Detailed description of the original: ${imageDescription}`,
              ...
              try {
                ...
                if (nanoModel) {
                  const inputParams = nanoModel.formatInput
                    ...
                    : { prompt: imageDescription, image_size: 'square_hd' };
        ...
        const isEditRequest = !isLikelyQuestion && !mediaIntent && !isAspectRatioRecreation && lastImageUrlInConversation && editImagePatterns.some(p => p.test(sanitizedContent));
        ...
        if (mediaIntent && !mediaFlow) {
          // Check user's quick-generate preference
            ...
            try {
              ...
              if (hasConfirmImageAttachment) {
                ...
                if (openaiApiKey) {
                  ...
                  for (const att of confirmImageAttachments.slice(0, 2)) {
                    if (att.base64) {
                      const base64Data = att.base64.startsWith('data:') ? att.base64 : `data:${att.mimeType || att.mime_type || 'image/png'};base64,${att.base64}`;
                      visionContent.push({ type: 'image_url', image_url: { url: base64Data, detail: 'low' } });
                    } else if (att.url) {
...
- Output ONLY the refined prompt text, nothing else`,
            ...
            // If user has image attachments, upload them NOW so the URL is available during confirmation
            let confirmSourceImageUrl = null;
            if (hasConfirmImageAttachment && mediaIntent === 'video') {
              const kieKey = process.env.KIE_API_KEY;
              if (kieKey) {
                const att = confirmImageAttachments[0];
                const mType = att.mimeType || att.mime_type || 'image/jpeg';
                const fileName = att.name || `confirm-image-${Date.now()}.${mType.split('/')[1] || 'jpg'}`;
                const dataUrl = att.base64?.startsWith('data:') ? att.base64 : `data:${mType};base64,${att.base64}`;
                ...
                try {
                  ...
                  if (uploadRes.ok) {
                    ...
                    if ((uploadData.success || uploadData.code === 200) && downloadUrl) {
                      confirmSourceImageUrl = downloadUrl;
                      console.log('[MediaConfirm] Pre-uploaded source image for video:', confirmSourceImageUrl.substring(0, 80));
                    }
                  }
                } catch (uploadErr) {
                  console.log('[MediaConfirm] Source image upload failed:', uploadErr.message);
                }
              }
            }
                ...
                if (imageFiles.length > 0) {
                  ...
                  if (imageFiles.length >= 2 && openaiApiKey) {
                    try {
                      ...
                      const visionContent = [
                        { type: 'text', text: `The user uploaded ${imageFiles.length} images and wants to create a composite image from them. Their request: "${content}"\n\nDescribe EACH image briefly (what it shows) and how they should be combined based on the user's request. Number each image.` }
                      ];
                      
                      for (let i = 0; i < imageAttachments.length; i++) {
                        const att = imageAttachments[i];
                        if (att.base64) {
                          const b64 = att.base64.startsWith('data:') ? att.base64 : `data:${att.mimeType || 'image/png'};base64,${att.base64}`;
                  ...
                  const compositePrompt = imageFiles.length >= 2
                    ? `CRITICAL: You MUST use the EXACT visual elements from ALL ${imageFiles.length} attached images as the base for this composite. Do NOT reimagine, redraw, or reinterpret any of the attached images — use them AS-IS.

${compositeContext ? `Image analysis:\n${compositeContext}\n\n` : ''}User's request: ${enhancedPrompt}

Rules:
- Each attached image contains a specific visual element (character, logo, object, etc.) that MUST appear in the final image EXACTLY as shown in the reference
- Preserve the EXACT art style, colors, proportions, and details of each reference image
- Compose these elements together in a natural scene as described by the user
- Do NOT create new versions of the characters/logos/objects — use the originals
- The final image should look like a professional composite of the actual reference images`
        ...
        if (fullContent.length > 0 && !streamVideoTaskSent) {
