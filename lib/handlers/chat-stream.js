...
async function handleChatStream(request) {
  ...
  if (!conv) {
    ...
    // Keep the verified project context available during this first request.
    // Without this, the first message falls back to the global imprint because
    // the newly inserted conversation is not reloaded until the next request.
    conv = newConv;
  }
