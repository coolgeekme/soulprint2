    ...
    const docStream = new ReadableStream({
      async start(controller) {
        try {
          ...
          controller.enqueue(encoder.encode(JSON.stringify({ 
            type: 'delta', 
            content: `\n\n✅ **${documentIntent.type.toUpperCase()} file generated successfully!**\n\n` +
                     `<a href="${dataUrl}" download="${fileName}" style="display: inline-block; padding: 8px 16px; background: #f97316; color: white; text-decoration: none; border-radius: 8px; font-weight: 600;">📥 Download ${fileName}</a>\n\n` +
                     `The file is ready. Click the button above to download.`
          }) + '\n'));
