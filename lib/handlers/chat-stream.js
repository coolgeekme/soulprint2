            ...
            const visionRes = await fetch('https://api.openai.com/v1/chat/completions', {
              ...
              body: JSON.stringify({
                ...
                messages: [
                  {
                    role: 'system',
                    content: `You are an expert image analyst. Describe this image in extraordinary detail so it can be precisely recreated by an AI image generator at a different aspect ratio (${targetAspectRatio}). Include:
...
Be extremely detailed and specific. The goal is to recreate this EXACT image but recomposed for a ${targetAspectRatio} aspect ratio, preserving ALL visual elements and the same artistic style.`
                  },
                  {
                    role: 'user',
                    content: [
            ...
            try {
              ...
              const editResult = await openai.images.edit({
                model: 'gpt-image-1',
                image: refFile,
                prompt: `Recreate this EXACT image recomposed for a ${targetAspectRatio} aspect ratio. Preserve every visual element, color, style, mood, and detail. Do NOT crop or shrink — create a new composition that naturally fills the ${targetAspectRatio} frame. Detailed description: ${imageDescription}`,
                n: 1,
                size: targetSize,
                quality: 'high',
              });
              ...
              if (editResult.data?.[0]) {
                if (editResult.data[0].url) {
                  newImageUrl = editResult.data[0].url;
              ...
              try {
                ...
                if (nanoModel) {
                  const inputParams = nanoModel.formatInput
                    ? nanoModel.formatInput(`Recreate the following image exactly as described, recomposed for ${targetAspectRatio} aspect ratio: ${imageDescription}`, targetAspectRatio)
            ...
            if (!newImageUrl && openaiApiKey) {
              try {
                ...
                const imgRes = await fetch('https://api.openai.com/v1/images/generations', {
                  ...
                  body: JSON.stringify({
                    model: 'dall-e-3',
                    prompt: `Recreate the following image exactly, recomposed for ${targetAspectRatio} aspect ratio: ${imageDescription}`,
                    n: 1,
                    size: dalleSize,
                    quality: 'hd',
                    style: 'natural',
