...
export async function handleAttachmentUpload(request) {
  try {
    ...
    if (cleanBase64.startsWith('data:')) {
      ...
      // Extract mime from data URL header if not explicitly provided
      if (!detectedMime && parts[0]) {
    ...
    if (!/^[A-Za-z0-9+/=\s]+$/.test(cleanBase64.substring(0, 200))) {
      return NextResponse.json({ error: 'Invalid base64 data' }, { status: 400 });
    }

    // Validate image MIME type
    const validImageTypes = ['image/jpeg', 'image/png', 'image/webp', 'image/gif', 'image/heic', 'image/heif', 'image/svg+xml', 'image/bmp', 'image/tiff'];
    if (detectedMime && !validImageTypes.includes(detectedMime)) {
      console.warn(`[AttachmentUpload] Non-image MIME type: ${detectedMime} — allowing upload but flagging`);
    }
