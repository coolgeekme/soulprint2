  ...
  if (openaiApiKey && imageBase64 && isComplexEdit) {
    try {
      ...
      if (editResult.data?.[0]?.url || editResult.data?.[0]?.b64_json) {
        ...
        if (editResult.data[0].b64_json && kieKey) {
        ...
        // If b64_json didn't work but we have a URL, persist it to permanent storage
        if (!resultUrl && editResult.data[0].url) {
          const tempUrl = editResult.data[0].url;
          console.log('[ImageEdit] Got temporary OpenAI URL, persisting to permanent storage...');
            ...
            try {
              ...
              if (dlResp.ok) {
                ...
                if (uploadRes.ok) {
                  ...
                  if (uploadData.success && uploadData.data?.downloadUrl) {
                    resultUrl = uploadData.data.downloadUrl;
                    console.log('[ImageEdit] Persisted to permanent URL:', resultUrl.substring(0, 80));
                  }
                }
              }
            } catch (persistErr) {
              console.log('[ImageEdit] Failed to persist URL:', persistErr.message);
            }
          }
          
          // Last resort: use temporary URL (will expire)
          if (!resultUrl) {
            console.log('[ImageEdit] WARNING: Using temporary OpenAI URL');
            resultUrl = tempUrl;
          }
        }
