...
async function getUserMemoriesForPrompt(db, userId) {
  const memories = await db.collection('user_memories')
    .find({ user_id: userId })
    .sort({ created_at: -1 }) // Most recent first — ensures fresh info is prioritized
    .limit(100) // Increased from 50 to support power users with rich memory profiles
    .toArray();
  
  // Sort by importance after retrieval (high > medium > low)
  const importanceOrder = { high: 0, medium: 1, low: 2 };
  memories.sort((a, b) => (importanceOrder[a.importance] || 2) - (importanceOrder[b.importance] || 2));
  ...
  if (memories.length > 0) {
    ...
    if (memorySections.length > 0) {
      ...
      const familySummary = familyRelated.length > 0 
        ? `\n### 👨‍👩‍👧‍👦 Family Quick Reference (${familyRelated.length} entries)\n${familyRelated.map(m => `- ${m.content}`).join('\n')}`
        : '';

      memoryContext = `\n# ⚡ LONG-TERM MEMORY — ${memories.length} STORED FACTS (READ EVERY SINGLE ONE)
**RULES:**
- When asked about ANY topic below, cite EVERY relevant entry — not just one
- If asked "how many kids/children", COUNT every child entry below — the answer is the TOTAL count
- Cross-reference ALL categories — family info may be in "relationships", "personal", OR "other"
- These are GROUND TRUTH from the user — never contradict or omit any
${familySummary}
...
When answering questions about ${displayName}, scan the ENTIRE list above. Do NOT stop after finding one match.`;
    }
  }
...
When ${displayName} asks about THEMSELVES (e.g., "how many kids do I have?", "what's my wife's name?"), you MUST:
...
4. **NEVER GUESS OR HALLUCINATE** — If not in memories, say "I don't have that information stored yet"
5. **CROSS-REFERENCE CATEGORIES** — Family info can be in "relationships", "personal", or "other"

**Example**: If asked "how many kids do I have?" and you see:
...
NOT: "You have 2 kids" or any other number. The memories are GROUND TRUTH.`;
  ...
  return fullPrompt + conversionContext + smbProContext;
}
