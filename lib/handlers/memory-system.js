...
- **Important**: When the user mentions times (like "3pm tomorrow" or "next Monday"), interpret them in their timezone (${userTimezone}). Always confirm the full date and time before creating calendar events.`;
  ...
  if (memories.length > 0) {
    ...
    if (memorySections.length > 0) {
      memoryContext = `\n## Long-Term Memory (CRITICAL — READ ALL)
These are verified facts about ${displayName} stored in their memory bank. You MUST:
1. **Reference ALL relevant memories** when answering questions about the user — never give partial answers
2. **Count correctly** — if there are 4 children listed, the user has 4 children, not 1
3. **Cross-reference categories** — a question about "family" should pull from relationships, personal, AND other categories
4. **Never contradict stored memories** — these are ground truth provided by the user
5. **When asked "do you remember..." or "what do you know about me"**, scan ALL categories below and cite specifics

${memorySections.join('\n\n')}

**Total memories stored: ${memories.length}** — Always give COMPLETE answers drawing from ALL relevant entries above.`;
    }
  }
