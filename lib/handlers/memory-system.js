...
// ── Memory freshness policy (memory drift) ────────────────────────────────────
// Staleness is detected DETERMINISTICALLY — no model calls. Volatile categories
// rot fast; everything else gets a longer leash. Flagged memories are qualified
// ("may be outdated") rather than silently dropped: stale != wrong.
const VOLATILE_MEMORY_CATEGORIES = ['work', 'goals', 'other'];
const VOLATILE_STALE_DAYS = 60;
const DEFAULT_STALE_DAYS = 180;

function memoryAgeDays(mem) {
  const ts = mem?.last_verified_at || mem?.updated_at || mem?.created_at;
  if (!ts) return null;
  return (Date.now() - new Date(ts).getTime()) / 86400000;
}

// Human-readable qualifier for an injected fact. Stale facts are qualified, not dropped.
function memoryFreshnessNote(mem) {
  const days = memoryAgeDays(mem);
  const flagged = mem?.review_state === 'needs_review' || isStaleMemory(mem);
  if (!flagged) return '';
  const age = days == null ? '' : ` (recorded ${Math.round(days)} days ago)`;
  return `${age} — may be outdated, confirm before relying on it`;
}

function isStaleMemory(mem) {
  if (mem?.review_state === 'needs_review') return true;
  const days = memoryAgeDays(mem);
  if (days == null) return false;
  const limit = VOLATILE_MEMORY_CATEGORIES.includes(mem?.category) ? VOLATILE_STALE_DAYS : DEFAULT_STALE_DAYS;
  return days > limit;
}
...
async function getUserMemoriesForPrompt(db, userId, currentContext = '') {
  ...
  const scoredMemories = allMemories.map(mem => {
    ...
    // 4. Freshness: bonus for fresh, decay for old, penalty for flagged.
    // Uses last_verified_at (set on write/edit/cleanup) falling back to created_at.
    const daysSinceVerified = memoryAgeDays(mem) ?? 999;
    if (daysSinceVerified < 30) score += 3;
    else if (daysSinceVerified < 90) score += 1;
    else score -= Math.min(12, Math.round(daysSinceVerified / 30)); // ~1pt per 30d, capped at 12
    if (mem.review_state === 'needs_review' || isStaleMemory(mem)) score -= 8;
    
    // 5. Direct name/entity matching (high value)
    // Extract potential names (capitalized words) — use the ORIGINAL case,
    // not the lowercased copy (was matching against lowercase = always empty).
    const contextNames = currentContext.match(/\b[A-Z][a-z]+\b/g) || [];
...
async function handleGetMemories(request) {
  ...
  return ok({
    memories: memories.map(m => ({
      ...
      review_state: m.review_state || (isStaleMemory(m) ? 'needs_review' : 'active'),
      is_stale: isStaleMemory(m),
    })),
...
// ─── Auto-Extraction (paid-only) ─────────────────────────────────────────
// Shared tier→autoExtraction capability map (mirrors auth route IDENTITY_TIERS)
const TIER_AUTO_EXTRACTION = {
  free: false,
  plus: true,
  pro: true,
  family: true,
...
async function hasAutoExtractionAccess(userId) {
  const db = await getDb();
  ...
  // Explicit identity-tier override
  if (user?.identity_tier && TIER_AUTO_EXTRACTION[user.identity_tier] === true) return true;
  ...
  if (sub && sub.plan_id && sub.plan_id !== 'free') return true;

  return false;
}

// POST /api/memories/extract - LLM-based auto-extraction (paid subscribers only)
async function handleExtractMemories(request) {
  const user = await authenticate(request);
  if (!user) return err('Unauthorized', 401);

  // Tier gate — defense in depth (extension also gates client-side)
  const allowed = await hasAutoExtractionAccess(user.id);
  if (!allowed) {
    return err('Auto-extraction requires a paid SoulPrint subscription', 403);
  }
  ...
  if (!batch.length) {
    return err('batch array is required', 400);
  }
  ...
  for (const item of batch) {
    const userMsg = item.userMsg || '';
    const assistantMsg = item.assistantMsg || '';
    if (!userMsg || !assistantMsg) continue;

    // LLM extraction (gpt-4o-mini) with negative-statement filtering
    const memories = await extractMemoriesFromMessage(userMsg, assistantMsg, user.id);
    if (!memories.length) continue;

    const saved = await saveExtractedMemories(db, user.id, memories, item.conversationId || null);
...
// ─── AI-Prompt Memory Import ────────────────────────────────────────────────
// POST /api/memories/import - import memories the AI dumps in reply to the
// "@soulprint-import" prompt. Two modes:
//   preview (default): { assistantMsg } -> parse + dedup, return candidates (nothing saved)
//   confirm: { memories: [...], confirm: true } -> save the user-approved subset
async function handleImportMemories(request) {
  const user = await authenticate(request);
  if (!user) return err('Unauthorized', 401);

  // Same paid gate as auto-extraction (defense in depth — extension also gates)
  const allowed = await hasAutoExtractionAccess(user.id);
  if (!allowed) {
    return err('Import requires a paid SoulPrint subscription', 403);
  }
  ...
  // ── Confirm mode: persist the user-approved subset ──
  if (body?.confirm) {
    const memories = Array.isArray(body.memories) ? body.memories : [];
    if (!memories.length) return err('memories array is required', 400);
    const saved = await saveImportedMemories(db, user.id, memories, body.conversationId || null);
    return ok({
      saved: saved.length,
      memories: saved.map(m => ({ id: m.id, content: m.content, category: m.category, importance: m.importance })),
    });
  }

  // ── Preview mode: parse the model reply and return candidates ──
  const assistantMsg = (body?.assistantMsg || '').trim();
  if (!assistantMsg) return err('assistantMsg is required', 400);

  const parsed = parseMemoryList(assistantMsg);

  const existingDocs = await db.collection('user_memories')
    .find({ user_id: user.id }, { projection: { content: 1 } })
    .toArray();
  const existingContents = existingDocs.map(d => d.content);

  const candidates = [];
  let duplicateCount = 0;
  for (const content of parsed) {
    if (isDuplicateMemory(content, existingContents)) { duplicateCount++; continue; }
    const { category, importance } = classifyMemory(content);
    candidates.push({ content, category, importance, duplicate: false });
  }
  ...
  return ok({
    ...
    duplicateCount,
    totalCount: existingContents.length,
  });
}

// Persist confirmed import candidates (re-checks negatives + duplicates server-side).
async function saveImportedMemories(db, userId, memories, conversationId = null) {
  const savedMemories = [];
  for (const mem of memories) {
    const content = (mem.content || '').trim();
    if (content.length < 5) continue;
    if (isNegativeStatement(content)) continue;

    const existing = await db.collection('user_memories').findOne({
      user_id: userId,
      content: { $regex: `^${content.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, $options: 'i' },
    });
  ...
  if (savedMemories.length > 0) invalidateSystemPromptCache(userId);
  return savedMemories;
}

// Parse a model's bullet-list reply into individual memory strings.
function parseMemoryList(text) {
  ...
  for (let line of lines) {
    ...
    if (/^(here'?s?|sure|of course|based on|as far as|the following|below|above)\b/i.test(line)) continue;
    // Drop the model talking TO the user, not stating facts
    if (/^(you should|you can|you could|you might|you may|do you|would you|could you|let me|i'?m|i'?ve|i have|i don'?t|i can'?t|i'?ll|here is|here are)\b/i.test(line)) continue;
    // Normalize second-person ("You work at X") -> third person ("Works at X")
    out.push(normalizeFactPhrasing(line));
  }
  if (!out.length) {
    // Fallback: model returned prose — split into sentences
    for (const s of String(text).split(/(?<=[.!?])\s+/)) {
      const t = s.trim();
      if (t.length >= 8 && t.length <= 300) out.push(t);
    }
  }
  const seen = new Set();
  return out.filter(c => {
    if (isNegativeStatement(c)) return false;
    const k = normalizeMemoryKey(c);
...
// Convert leading second-person facts into third-person memory fragments.
function normalizeFactPhrasing(line) {
  const m = line.match(/^you\s+(\w+)\b(.*)$/i);
  if (!m) return line;
  const verbMap = {
    are: 'is', were: 'was',
    work: 'works', live: 'lives', own: 'owns', run: 'runs', manage: 'manages',
    lead: 'leads', operate: 'operates', study: 'studies', teach: 'teaches',
    build: 'builds', design: 'designs', develop: 'develops', create: 'creates',
    write: 'writes', have: 'has', use: 'uses', speak: 'speaks', know: 'knows',
    believe: 'believes', think: 'thinks', feel: 'feels', prefer: 'prefers',
    like: 'likes', love: 'loves', hate: 'hates', enjoy: 'enjoys', dislike: 'dislikes',
    want: 'wants', need: 'needs', play: 'plays', coach: 'coaches', volunteer: 'volunteers',
  };
  const verb = m[1].toLowerCase();
  const replacement = verbMap[verb];
  if (!replacement) return line;
  return replacement.charAt(0).toUpperCase() + replacement.slice(1) + m[2];
}

// Exact or near-identical dedup against existing memories (token-overlap).
function isDuplicateMemory(content, existingContents) {
  const key = normalizeMemoryKey(content);
  const a = tokenizeMemory(content);
  for (const existing of existingContents) {
    if (normalizeMemoryKey(existing) === key) return true;
    const b = tokenizeMemory(existing);
    if (!b.length) continue;
    const inter = a.filter(t => b.includes(t));
    if (!inter.length) continue;
    const union = new Set([...a, ...b]);
    if (inter.length / union.size > 0.7) return true;
  }
  return false;
}

function normalizeMemoryKey(content) {
  return content.toLowerCase().replace(/[^\w\s]/g, '').replace(/\s+/g, ' ').trim();
}

function tokenizeMemory(content) {
  return (content.toLowerCase().match(/[a-z0-9']+/g) || []).filter(t => t.length > 2);
}

function isNegativeStatement(content) {
  const c = content.toLowerCase();
  const patterns = [
    /\bis not stored\b/i, /\bnot in memory\b/i,
    /\bdon'?t have (?:information|data|details|memory)\b/i,
    /\bis not (?:known|available|recorded)\b/i,
    /\bno (?:information|data|details|memory)\b/i,
    /\bdidn'?t mention\b/i,
...
// Light keyword classification so imported memories recall well.
function classifyMemory(content) {
  const c = content.toLowerCase();
  const rules = [
    ['health', /\b(allerg|medication|condition|diagnos|surgery|therapy|doctor|prescri|blood|heart|diabet|asthma|mental|anxiety|depression|pain|injury|pregnan)/i],
    ['relationships', /\b(wife|husband|spouse|partner|girlfriend|boyfriend|fianc|married|daughter|son|child|kid|mother|father|mom|dad|sister|brother|parent|family|friend|cousin|aunt|uncle|grandma|grandpa)/i],
    ['work', /\b(work|job|career|company|startup|business|cto|ceo|founder|co-founder|employee|employer|boss|manager|client|project|profession|industry|team|salary|promot|hire|quit|laid off)/i],
    ['goals', /\b(goal|aspir|dream|plan to|want to|hoping to|working toward|aim|ambition|resolution|target|saving up|saving for)/i],
    ['preferences', /\b(prefer|favorite|favourite|love|like|enjoy|hate|dislike|can'?t stand|avoid|fan of|into)/i],
  ];
  let category = 'personal';
  for (const [cat, re] of rules) { if (re.test(c)) { category = cat; break; } }
  const importance = /\b(allerg|medication|condition|diagnos|surgery|emergency|pregnan|diabet|heart|severe|anaphyl)/i.test(c) ? 'high' : 'medium';
...
async function handleUpdateMemory(request, memoryId) {
  ...
  const updates = { updated_at: new Date(), last_verified_at: new Date(), review_state: 'active', flagged_at: null };
...
**CRITICAL**: For factual queries about the *outside world* (news, prices, current events, general knowledge, "what is X", "how do I"), search first — never rely solely on training data. Cite sources with dates. If search results conflict with your training data, trust the search results.
**BUT** for questions about *the user themselves* — their history, preferences, identity, relationships, what they've done or own — answer from the Memories and profile context above FIRST. Do NOT web-search those; the web does not know this user, and the memories are authoritative for personal facts.
...
- **Important**: When the user mentions times (like "3pm tomorrow" or "next Monday"), interpret them in their timezone (${userTimezone}). Always confirm the full date and time before creating calendar events.`;
  ...
  // Build long-term memory context — pass the user's current message so the
  // smart-retrieval relevance scoring actually runs (was silently falling back
  // to "top 100 by recency", which buried relevant memories).
  let memoryContext = '';
  const memories = await getUserMemoriesForPrompt(db, userId, messageContext.content || '');
  if (memories.length > 0) {
    ...
    if (memorySections.length > 0) {
      ...
      const flatList = memories.map((m, i) => {
        const note = memoryFreshnessNote(m);
        return `${i+1}. [${m.category.toUpperCase()}] ${m.content}${note ? ` [${note}]` : ''}`;
      }).join('\n');
      ...
      const familySummary = familyRelated.length > 0 
        ? `${familyCriticalSummary}\n### 👨‍👩‍👧‍👦 Family Memory Details (${familyRelated.length} raw entries)\n${familyRelated.map(m => `- ${m.content}${memoryFreshnessNote(m) ? ` [${memoryFreshnessNote(m)}]` : ''}`).join('\n')}`
        : '';
