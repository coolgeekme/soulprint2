...
const DEFAULT_PLANS = [
  {
    ...
    sort_order: 0,
    features: {
      chat_models: ['gpt-4o-mini', 'haiku', 'gemini-flash'],
      premium_chat: false,
      images_per_month: 20,
  ...
  },
  {
    ...
    features: {
      chat_models: ['gpt-4o-mini', 'haiku', 'gemini-flash'],
      premium_chat: true,
      premium_chat_models: ['gpt-4o', 'claude-opus', 'claude-sonnet'],
      images_per_month: 20,
      ...
      memory_soulprint: true,
      assessment: true,
      support_level: 'email_48hr',
      data_retention_days: 90,
    },
    add_ons: {
      premium_chat_per_msg: 0.15,
      image_premium_per_gen: 0.60,
      image_basic_per_gen: 0.20,
      voice_per_min: 0.40,
  ...
  },
  {
    ...
    slug: 'power',
    price_monthly: 99.00,
    price_annual: 950.40, // 99 * 12 * 0.80
    stripe_price_id_monthly: null,
    stripe_price_id_annual: null,
    is_active: true,
    sort_order: 2,
    features: {
      ...
      conversation_search: true,
      api_access: true,
      multi_platform: ['web', 'telegram', 'api'],
      memory_soulprint: true,
      assessment: true,
      support_level: 'priority_24hr',
...
async function seedPlans() {
  ...
  for (const plan of DEFAULT_PLANS) {
    ...
    if (!existing) {
      await col.insertOne({ ...plan, created_at: new Date(), updated_at: new Date() });
      console.log(`[Pricing] Seeded plan: ${plan.name}`);
    }
  }

  // Seed video credit packs
  const packCol = await getCollection('video_credit_packs');
  for (const pack of VIDEO_CREDIT_PACKS) {
    const existing = await packCol.findOne({ id: pack.id });
    if (!existing) {
      await packCol.insertOne({ ...pack, is_active: true, created_at: new Date() });
    }
  }
...
async function syncPlansToStripe() {
  ...
  // Sync video credit packs
  const packs = await packCol.find({ is_active: true }).toArray();
  for (const pack of packs) {
    if (pack.stripe_price_id) continue;
    try {
      const product = await stripe.products.create({
        name: `SoulPrint Video Credits — ${pack.name}`,
        description: `${pack.credits} video credits`,
...
async function createCheckoutSession(userId, planId, billingPeriod, originUrl, discountCode = null) {
  ...
  const sessionParams = {
    ...
    success_url: `${originUrl}/chat?subscription_success=true&session_id={CHECKOUT_SESSION_ID}`,
...
async function createCreditPackCheckout(userId, packId, originUrl) {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: `${originUrl}/chat?credits_success=true&session_id={CHECKOUT_SESSION_ID}`,
...
async function processSuccessfulPayment(txn, session) {
  if (txn.type === 'subscription') {
    ...
    console.log(`[Pricing] Added ${txn.credits} credits for user ${txn.user_id}`);
  }
}
...
async function getUserUsageSummary(userId) {
  ...
  return {
    ...
    limits: {
      ...
      videos_per_month: plan?.features?.videos_per_month || (plan?.features?.videos_lifetime ? 'lifetime:1' : null),
...
async function createDiscountCode({
  ...
  if (type === 'percent_off') {
    const coupon = await stripe.coupons.create({
      percent_off: value,
      duration: is_lifetime_deal ? 'forever' : 'once',
