...
const DEFAULT_PLANS = [
  ...
  },
  {
    ...
    slug: 'base',
    price_monthly: 19,
    price_annual: 182.40, // 19 * 12 * 0.80 (20% discount)
    stripe_price_id_monthly: null,
    stripe_price_id_annual: null,
    is_active: true,
    sort_order: 1,
  ...
  },
  {
    ...
    slug: 'plus',
    price_monthly: 39,
    price_annual: 374.40, // 39 * 12 * 0.80 (20% discount)
    stripe_price_id_monthly: 'price_1TmEuYPK7jhQlR2aV6VU6QzT',
    stripe_price_id_annual: 'price_1TmEvEPK7jhQlR2akqKO6YnF',
    is_active: true,
    sort_order: 1.5,
    features: {
      chat_model_tier: 'all',  // Standard + Premium models
      premium_chat: true,
      premium_chat_unlimited: true, // Unlimited premium messages
      ...
      video_models: ['kling-3.0', 'veo-ultra'],
      pdfs_per_month: 50,
      voice_chat: true,
      voice_unlimited: true, // Unlimited voice
      file_analysis_basic: true,
      file_analysis_advanced: true,
      file_max_pages: null,
      conversation_search: true,
    ...
    description: 'Power user features at a mid-tier price.',
  },
  {
    id: 'power',
    name: 'Power',
    slug: 'power',
    price_monthly: 97,
    price_annual: 931.20, // 97 * 12 * 0.80 (20% discount)
    stripe_price_id_monthly: null,
    stripe_price_id_annual: null,
    is_active: true,
    sort_order: 2,
...
async function syncPlansToStripe(forceResyncPlanIds = []) {
  const plans = await getPlans(true);
  const col = await getCollection('subscription_plans');
  const packCol = await getCollection('media_credit_packs');
  const results = [];

  // Auto-detect test→live mode switch: if DB has test-mode IDs but we're using live keys, clear them
  const isLiveMode = process.env.STRIPE_SECRET_KEY?.startsWith('sk_live_');
  for (const plan of plans) {
    if (plan.stripe_product_id?.startsWith('prod_') && plan.stripe_price_id_monthly?.startsWith('price_')) {
      // Try to retrieve — if it fails (wrong mode), we need to clear
      try {
        await stripe.products.retrieve(plan.stripe_product_id);
      } catch (e) {
        if (e.code === 'resource_missing' || e.statusCode === 404) {
          console.log(`[Stripe] Product ${plan.stripe_product_id} not found in ${isLiveMode ? 'live' : 'test'} mode — clearing for re-creation`);
          if (!forceResyncPlanIds.includes(plan.id)) {
            forceResyncPlanIds.push(plan.id);
          }
        }
      }
    }
  ...
  // Force re-sync: clear existing Stripe price IDs for specified plans
  if (forceResyncPlanIds.length > 0) {
    for (const planId of forceResyncPlanIds) {
      const existingPlan = await col.findOne({ id: planId });
      if (existingPlan) {
        // Try to archive old Stripe prices (may fail if they belong to a different mode)
        try {
          if (existingPlan.stripe_price_id_monthly) {
            await stripe.prices.update(existingPlan.stripe_price_id_monthly, { active: false });
            console.log(`[Stripe] Archived old monthly price for ${planId}: ${existingPlan.stripe_price_id_monthly}`);
          }
          if (existingPlan.stripe_price_id_annual) {
            await stripe.prices.update(existingPlan.stripe_price_id_annual, { active: false });
            console.log(`[Stripe] Archived old annual price for ${planId}: ${existingPlan.stripe_price_id_annual}`);
          }
        } catch (archiveErr) {
          console.warn(`[Stripe] Could not archive old prices for ${planId} (likely different mode):`, archiveErr.message);
        }
        // Clear all Stripe IDs and update to latest seed price
        await col.updateOne({ id: planId }, { $set: {
          stripe_product_id: null,
          stripe_price_id_monthly: null,
          stripe_price_id_annual: null,
          price_monthly: planId === 'power' ? 99.01 : existingPlan.price_monthly,
          price_annual: planId === 'power' ? 950.50 : existingPlan.price_annual,
          updated_at: new Date(),
        }});
        console.log(`[Stripe] Cleared all Stripe IDs for ${planId}, ready for live re-creation`);
      }
    }
    // Reload plans after clearing
    const updatedPlans = await col.find({ is_active: true }).sort({ sort_order: 1 }).toArray();
    plans.length = 0;
    plans.push(...updatedPlans);
  }
...
async function createCheckoutSession(userId, planId, billingPeriod, originUrl, discountCode = null) {
  ...
  if (customerId) {
    ...
    try {
      ...
      console.log(`[Stripe] Customer ${customerId} not found (likely test-mode). Creating new live customer.`);
      customerId = null; // Force re-creation
    }
  }
  if (!customerId) {
  ...
  const sessionParams = {
    ...
    success_url: `${originUrl}/thank-you?plan=${planId}&session_id={CHECKOUT_SESSION_ID}`,
...
async function createCreditPackCheckout(userId, packId, originUrl) {
  ...
  if (customerId) {
    try {
      ...
      console.log(`[Stripe] Customer ${customerId} not found in current mode. Creating new.`);
      customerId = null;
    }
  }
  if (!customerId) {
...
async function processSuccessfulPayment(txn, session) {
  if (txn.type === 'subscription') {
    ...
    if (txn.discount_code) {
      await redeemDiscountCode(txn.discount_code);
      console.log(`[Pricing] Redeemed discount code: ${txn.discount_code}`);
    }
...
async function deleteDiscountCode(codeId) {
  const col = await getCollection('discount_codes');
  return col.deleteOne({ id: codeId });
}
