...
export async function handleSupportLogin(request) {
  try {
    ...
    const token = generateToken({ id: user.id, email: user.email, role: 'support', name: user.name });
...
async function authenticateSupport(request) {
  try {
    ...
    if (!decoded || decoded.role !== 'support') {
      // Also allow admin/superadmin
      if (decoded?.role === 'admin' || decoded?.role === 'superadmin') return decoded;
      return null;
    }
    return decoded;
  } catch {
...
Only include fix_actions for data_fix type. For code_fix, describe what code changes are needed in the summary.` },
