...
async function sendTelegramAction(chatId, token, action = 'typing') {
...
async function transcribeVoiceMessage(fileId, botToken) {
...
async function handleTelegramWebhook(request) {
  ...
  if (!message?.chat?.id) return ok({ ok: true });

  const chatId = message.chat.id;
  const username = message.from?.username || null;

  // Detect voice/audio messages for transcription
  const voiceFile = message.voice || message.audio || message.video_note;
  const hasText = !!message.text;

  if (!hasText && !voiceFile) return ok({ ok: true }); // Ignore unsupported message types
  ...
  if (voiceFile) {
    ...
    try {
      await sendTelegramAction(chatId, botToken, 'typing');
      text = await transcribeVoiceMessage(voiceFile.file_id, botToken);
      if (!text || !text.trim()) {
        await sendTelegramMessage(chatId, botToken, '🎤 I couldn\'t detect any speech in that voice message. Please try again.');
        return ok({ ok: true });
      }
    } catch (transcribeErr) {
      console.error('Voice transcription error:', transcribeErr);
      await sendTelegramMessage(chatId, botToken, '⚠️ Sorry, I couldn\'t transcribe that voice message. Please try sending a text message instead.');
      return ok({ ok: true });
    }
  } else {
  ...
  await db.collection('messages').insertOne({
    ...
    content: text,
    source: 'telegram',
    input_type: voiceFile ? 'voice' : 'text',
    voice_duration: voiceFile?.duration || null,
    created_at: new Date(),
  });
  ...
  try {
    ...
    // Prepend transcription when it was a voice message
    const fullReply = voiceFile
      ? `🎤 *Heard:* "${text}"\n\n${reply}`
      : reply;
    ...
    await sendTelegramMessage(chatId, botToken, fullReply);
  } catch (aiErr) {
