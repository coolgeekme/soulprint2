...
async function fetchGoogleContextForChat(userId, query) {
  ...
  // Also check Composio connections for Gmail/Calendar
  let composioGmailAccounts = [];
  if (!connections || connections.length === 0) {
    try {
      const { getActiveComposioAccounts } = await import('@/lib/handlers/composio');
      const composioAccounts = await getActiveComposioAccounts(userId);
      composioGmailAccounts = composioAccounts['GMAIL'] || [];
      const composioCalendarAccounts = composioAccounts['GOOGLECALENDAR'] || [];
      
      if (composioGmailAccounts.length === 0 && composioCalendarAccounts.length === 0) {
        return null; // No native or Composio Google connections
      }
      
      // Build context from Composio connections
      const allComposioGoogle = [...composioGmailAccounts, ...composioCalendarAccounts];
      return {
        hasGoogleConnected: true,
        isComposio: true,
        accounts: composioGmailAccounts.map(a => ({
          email: a.alias || 'connected Gmail',
          name: a.alias || 'Gmail',
          isDefault: true,
        })),
        gmail: null,
...
function formatGoogleContextForPrompt(googleContext) {
  ...
  if (googleContext.gmail && googleContext.gmail.length > 0) {
    ...
    googleContext.gmail.forEach((email, i) => {
      ...
      if (email.body) {
        contextStr += `Content: ${email.body.slice(0, 300)}${email.body.length > 300 ? '...' : ''}\n`;
      }
    });
  }
...
// ── Execute Google Actions via Composio ────────────────────────────────────
// Maps our tool names to Composio action slugs and handles execution
async function executeGoogleActionViaComposio(toolName, composioAccountId, connectionEmail, args) {
  const { executeComposioAction } = await import('@/lib/handlers/composio');
  
  try {
    switch (toolName) {
      case 'search_emails': {
        const { query, max_results = 10 } = args;
        console.log(`[Composio Gmail] Searching emails: "${query}" (max: ${max_results}) via account ${composioAccountId}`);
        const result = await executeComposioAction('GMAIL_FETCH_EMAILS', composioAccountId, {
          query: query,
          max_results: max_results,
        });
        
        // Parse Composio response into our standard format
        const messages = result?.data?.messages || result?.response_data?.messages || result?.data?.result || [];
        if (Array.isArray(messages) && messages.length > 0) {
          const formatted = messages.slice(0, max_results).map(m => ({
            id: m.id || m.messageId,
            ...
            date: m.date || m.internalDate || m.receivedDateTime || '',
            snippet: m.snippet || m.bodyPreview || m.body?.substring(0, 200) || '',
          }));
          return {
            success: true,
            message: `Found ${formatted.length} email(s) matching "${query}" in ${connectionEmail}`,
            emails: formatted,
          };
        }
        
        // If the raw result has useful data, try to extract it
        if (result?.data || result?.response_data) {
          const raw = result.data || result.response_data;
          return {
            success: true,
            message: `Search completed for "${query}" in ${connectionEmail}`,
            raw_result: typeof raw === 'string' ? raw.substring(0, 2000) : JSON.stringify(raw).substring(0, 2000),
          };
        }
        
        return { success: true, message: `No emails found matching "${query}" in ${connectionEmail}` };
      }

      case 'read_email': {
        const { message_id } = args;
        console.log(`[Composio Gmail] Reading email: ${message_id}`);
        const result = await executeComposioAction('GMAIL_GET_MESSAGE', composioAccountId, {
        ...
        return {
          success: true,
          message: `Email retrieved from ${connectionEmail}`,
          email: {
            id: data?.id || message_id,
            subject: data?.subject || data?.payload?.headers?.find(h => h.name === 'Subject')?.value || '',
            from: data?.from || data?.payload?.headers?.find(h => h.name === 'From')?.value || '',
            body: data?.body || data?.snippet || JSON.stringify(data).substring(0, 3000),
          },
        };
      }
      ...
      case 'send_email': {
        ...
        console.log(`[Composio Gmail] Sending email to ${to} from ${connectionEmail}`);
        const result = await executeComposioAction('GMAIL_SEND_EMAIL', composioAccountId, {
          recipient_email: to,
          subject: subject,
          body: body,
        });
        return {
          success: true,
          message: `✅ Email sent successfully to ${to} from ${connectionEmail}`,
          details: { to, subject, messageId: result?.data?.id },
        };
      }

      case 'create_calendar_event': {
        const { summary, description, start, end, attendees } = args;
        console.log(`[Composio Calendar] Creating event: "${summary}"`);
        const result = await executeComposioAction('GOOGLECALENDAR_CREATE_EVENT', composioAccountId, {
          summary,
      ...
      default:
        return { success: false, error: `Action '${toolName}' is not yet supported via Composio. Supported: search_emails, read_email, send_email, create_calendar_event.` };
    }
  } catch (err) {
    console.error(`[Composio GoogleAction] ${toolName} failed:`, err.message);
    return { success: false, error: `Failed to execute ${toolName} via connected account: ${err.message}` };
  }
}
...
async function executeGoogleAction(userId, toolName, args) {
  ...
  let composioAccountId = null;
  
  if (account_email) {
    accessToken = await getTokenForAccount(userId, account_email);
    connectionEmail = account_email;
    if (!accessToken) {
      // Try Composio fallback for this email
      try {
        const { getActiveComposioAccounts } = await import('@/lib/handlers/composio');
        const composioAccounts = await getActiveComposioAccounts(userId);
        const gmailAccounts = composioAccounts['GMAIL'] || [];
        const match = gmailAccounts.find(a => a.alias && a.alias.toLowerCase() === account_email.toLowerCase());
        if (match) {
          useComposio = true;
          composioAccountId = match.id;
          connectionEmail = account_email;
        } else if (gmailAccounts.length > 0) {
          // Use first available Composio Gmail account
          useComposio = true;
          composioAccountId = gmailAccounts[0].id;
          connectionEmail = gmailAccounts[0].alias || account_email;
        }
      } catch (e) {
        console.error('[GoogleAction] Composio fallback failed:', e.message);
      }
      if (!useComposio) {
        return { success: false, error: `Google account ${account_email} not connected or token expired. Please connect your Google account in Settings > Integrations.` };
      }
    }
  } else {
    ...
    if (connection) {
      ...
      try {
        ...
        if (gmailAccounts.length > 0) {
          useComposio = true;
          composioAccountId = gmailAccounts[0].id;
          connectionEmail = gmailAccounts[0].alias || 'connected account';
        }
      } catch (e) {
        console.error('[GoogleAction] Composio fallback failed:', e.message);
      }
  ...
  if (useComposio && composioAccountId) {
    return executeGoogleActionViaComposio(toolName, composioAccountId, connectionEmail, args);
  }

  // ── NATIVE GOOGLE EXECUTION PATH ─────────────────────────────────────────
...
async function userHasGoogleConnected(userId) {
  const db = await getDb();
  // Check native Google OAuth connections
  const nativeConnection = await db.collection('google_connections').findOne({ user_id: userId });
  if (nativeConnection) return true;
  
  // Also check Composio connections for Gmail/Calendar
  try {
    const { getActiveComposioAccounts } = await import('@/lib/handlers/composio');
    const composioAccounts = await getActiveComposioAccounts(userId);
    if (composioAccounts['GMAIL'] || composioAccounts['GOOGLECALENDAR']) return true;
  } catch (e) {
    // Composio not configured or unavailable — that's fine
  }
  return false;
}
