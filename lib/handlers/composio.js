...
const SUPPORTED_TOOLKITS = {
  ...
  GOOGLEDRIVE: {
    name: 'Google Drive',
    icon: '📁',
    description: 'Access, upload, and manage files',
    category: 'productivity',
...
async function handleComposioToolCall(toolName, args, activeAccounts) {
  try {
    switch (toolName) {
      ...
      // ── Google Drive ──────────────────────────────────────────────────
      case 'composio_drive_list_files': {
        const acc = resolveAccountSelection(activeAccounts.GOOGLEDRIVE, args.account);
        if (!acc) return { error: 'No Google Drive connected' };
        console.log(`[Composio] Listing Drive files for: ${acc.alias || acc.id.slice(0, 12)}`);
        const result = await executeComposioAction('GOOGLEDRIVE_LIST_FILES', acc.id, {
          page_size: Math.min(args.max_results || 10, 20),
          q: args.query || '',
        });
      ...
      case 'composio_drive_upload_file': {
        const acc = resolveAccountSelection(activeAccounts.GOOGLEDRIVE, args.account);
        if (!acc) return { error: 'No Google Drive connected' };
        console.log(`[Composio] Uploading file to Drive: ${acc.alias || acc.id.slice(0, 12)}`);
        const result = await executeComposioAction('GOOGLEDRIVE_CREATE_FILE', acc.id, {
          name: args.filename,
          content: args.content,
          parent_folder_id: args.folder_id || 'root',
        });
        return { success: true, account_used: acc.alias || acc.id.slice(0, 12), file: result?.data };
      }

      // ── Google Docs ───────────────────────────────────────────────────
      case 'composio_docs_create': {
        const acc = resolveAccountSelection(activeAccounts.GOOGLEDOCS, args.account);
        if (!acc) return { error: 'No Google Docs connected' };
        console.log(`[Composio] Creating Google Doc: ${acc.alias || acc.id.slice(0, 12)}`);
        const result = await executeComposioAction('GOOGLEDOCS_CREATE_DOCUMENT', acc.id, {
          title: args.title,
        });
        return { success: true, account_used: acc.alias || acc.id.slice(0, 12), document: result?.data };
      }

      case 'composio_docs_read': {
        const acc = resolveAccountSelection(activeAccounts.GOOGLEDOCS, args.account);
        if (!acc) return { error: 'No Google Docs connected' };
        console.log(`[Composio] Reading Google Doc: ${acc.alias || acc.id.slice(0, 12)}`);
        const result = await executeComposioAction('GOOGLEDOCS_GET_DOCUMENT', acc.id, {
      ...
      case 'composio_docs_update': {
        const acc = resolveAccountSelection(activeAccounts.GOOGLEDOCS, args.account);
        if (!acc) return { error: 'No Google Docs connected' };
        console.log(`[Composio] Updating Google Doc: ${acc.alias || acc.id.slice(0, 12)}`);
        const result = await executeComposioAction('GOOGLEDOCS_UPDATE_DOCUMENT', acc.id, {
          document_id: args.document_id,
          requests: args.requests,
        });
        return { success: true, account_used: acc.alias || acc.id.slice(0, 12), result: result?.data };
      }

      // ── Google Sheets ─────────────────────────────────────────────────
      case 'composio_sheets_create': {
        const acc = resolveAccountSelection(activeAccounts.GOOGLESHEETS, args.account);
        if (!acc) return { error: 'No Google Sheets connected' };
        console.log(`[Composio] Creating Google Sheet: ${acc.alias || acc.id.slice(0, 12)}`);
        const result = await executeComposioAction('GOOGLESHEETS_CREATE_SPREADSHEET', acc.id, {
          title: args.title,
        });
        return { success: true, account_used: acc.alias || acc.id.slice(0, 12), spreadsheet: result?.data };
      }

      case 'composio_sheets_read': {
        const acc = resolveAccountSelection(activeAccounts.GOOGLESHEETS, args.account);
        if (!acc) return { error: 'No Google Sheets connected' };
        console.log(`[Composio] Reading Google Sheet: ${acc.alias || acc.id.slice(0, 12)}`);
        const result = await executeComposioAction('GOOGLESHEETS_GET_VALUES', acc.id, {
          spreadsheet_id: args.spreadsheet_id,
          range: args.range || 'A1:Z1000',
        });
        return {
          account_used: acc.alias || acc.id.slice(0, 12),
      ...
      case 'composio_sheets_update': {
        const acc = resolveAccountSelection(activeAccounts.GOOGLESHEETS, args.account);
        if (!acc) return { error: 'No Google Sheets connected' };
        console.log(`[Composio] Updating Google Sheet: ${acc.alias || acc.id.slice(0, 12)}`);
        const result = await executeComposioAction('GOOGLESHEETS_UPDATE_VALUES', acc.id, {
          spreadsheet_id: args.spreadsheet_id,
          range: args.range,
          values: args.values,
        });
      ...
      // ── Google Contacts ───────────────────────────────────────────────
      case 'composio_contacts_list': {
        const acc = resolveAccountSelection(activeAccounts.GOOGLECONTACTS, args.account);
        if (!acc) return { error: 'No Google Contacts connected' };
        console.log(`[Composio] Listing contacts: ${acc.alias || acc.id.slice(0, 12)}`);
        const result = await executeComposioAction('GOOGLECONTACTS_LIST_CONTACTS', acc.id, {
          page_size: Math.min(args.max_results || 10, 50),
        });
