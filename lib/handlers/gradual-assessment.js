...
import { invalidatePersonaProfile } from '@/lib/handlers/persona-dna';
...
async function handleGetAssessmentNudge(request) {
  ...
  // Steady cadence: show after every 4 messages
...
async function handleSubmitNudgeAnswer(request) {
  ...
  // Invalidate persona profile cache so the AI adapts to new assessment data
...
async function handleSubmitGradualAnswer(request) {
  ...
  // Use persona profile invalidation instead of direct cache access
  await invalidatePersonaProfile(db, user.id);
...
// ============================================================
// INLINE CHAT-BASED ASSESSMENT
// These functions let the AI naturally weave personality questions
// into conversations instead of using a separate overlay.
// ============================================================

const INLINE_MESSAGE_CADENCE = 5; // Ask inline question every 5 messages

/**
 * getInlineAssessmentQuestion(db, userId)
 * Returns the next unanswered assessment question if it's time to ask one inline.
 * Called from chat-stream.js before building the AI response.
 * Returns: { question, questionId, trait, lowLabel, highLabel } or null
 */
async function getInlineAssessmentQuestion(db, userId) {
  try {
    // Get or create inline progress tracker
    let tracker = await db.collection('inline_assessment_tracker').findOne({ user_id: userId });
    if (!tracker) {
      tracker = {
        user_id: userId,
      ...
      await db.collection('inline_assessment_tracker').insertOne(tracker);
    }

    // If there's a pending question (asked but not yet answered), don't ask another
    if (tracker.pending_question_id) return null;

    // Check cadence
    if ((tracker.messages_since_last || 0) < INLINE_MESSAGE_CADENCE) return null;

    // Get all answered question IDs from ALL sources (slider, nudge, inline, full assessment)
    const answeredFromSliders = await db.collection('assessment_slider_answers')
      .find({ user_id: userId }).toArray();
    const answeredFromNudge = await db.collection('assessment_nudge_progress')
      .findOne({ user_id: userId });
    
    const answeredIds = new Set();
    answeredFromSliders.forEach(a => answeredIds.add(a.question_id));
    if (answeredFromNudge?.answered_question_ids) {
      answeredFromNudge.answered_question_ids.forEach(id => answeredIds.add(id));
    }
    (tracker.asked_inline_ids || []).forEach(id => answeredIds.add(id));
    ...
    // Mark as pending
    await db.collection('inline_assessment_tracker').updateOne(
      { user_id: userId },
      { 
        $set: { 
          pending_question_id: question.id,
          pending_question_asked_at: new Date(),
          messages_since_last: 0,
          updated_at: new Date(),
        }
      }
    ...
    console.log(`[InlineAssessment] Asking user ${userId.substring(0,8)}... question: ${question.id} (${question.trait})`);
    ...
    console.error('[InlineAssessment] Error getting question:', err.message);
    return null;
  }
}

/**
 * processInlineAssessmentAnswer(db, userId, userMessage)
 * Called BEFORE generating a new AI response. Checks if there's a pending 
 * inline question and interprets the user's message as an answer.
 * Returns: { answered: true, trait, value } or { answered: false }
 */
async function processInlineAssessmentAnswer(db, userId, userMessage) {
  try {
    const tracker = await db.collection('inline_assessment_tracker').findOne({ user_id: userId });
    if (!tracker?.pending_question_id) {
      // No pending question — just increment message counter
      await db.collection('inline_assessment_tracker').updateOne(
        { user_id: userId },
        { 
          $inc: { messages_since_last: 1 },
          $set: { updated_at: new Date() },
          $setOnInsert: { user_id: userId, pending_question_id: null, asked_inline_ids: [], created_at: new Date() }
        },
        { upsert: true }
      );
    ...
    // There IS a pending question — try to interpret the user's answer
    const question = SLIDER_QUESTIONS.find(q => q.id === tracker.pending_question_id);
    if (!question) {
      // Question not found, clear pending
      await db.collection('inline_assessment_tracker').updateOne(
        { user_id: userId },
    ...
    // Check if the message is too short or clearly unrelated (skip interpretation)
    const msg = (userMessage || '').trim().toLowerCase();
    if (msg.length < 3) {
      return { answered: false };
    }

    // Use a simple heuristic + keyword analysis to interpret the answer
    // Maps the user's response to a 0-100 scale
    const value = interpretAnswer(msg, question);
    
    if (value !== null) {
      // Store the answer
      await db.collection('assessment_slider_answers').insertOne({
        id: uuidv4(),
        user_id: userId,
        ...
        value: value,
        source: 'inline_chat',
        raw_answer: userMessage.substring(0, 500),
        answered_at: new Date(),
      });

      // Clear pending and add to asked list
      await db.collection('inline_assessment_tracker').updateOne(
        { user_id: userId },
        {
          $set: { pending_question_id: null, updated_at: new Date() },
          $addToSet: { asked_inline_ids: question.id },
        }
      );

      // Also update nudge progress so the nudge system knows this was answered
      await db.collection('assessment_nudge_progress').updateOne(
        { user_id: userId },
      ...
      console.log(`[InlineAssessment] User ${userId.substring(0,8)}... answered ${question.id}: ${value}/100 (source: inline_chat)`);
      return { answered: true, trait: question.trait, value, questionId: question.id };
    }

    // If we can't interpret after 3 messages, clear the pending question
    const pendingAge = tracker.pending_question_asked_at 
      ? Date.now() - new Date(tracker.pending_question_asked_at).getTime()
      : 0;
    if (pendingAge > 5 * 60 * 1000) { // 5 minutes timeout
      await db.collection('inline_assessment_tracker').updateOne(
        { user_id: userId },
        { $set: { pending_question_id: null } }
      );
    ...
    console.error('[InlineAssessment] Error processing answer:', err.message);
    return { answered: false };
  }
}

/**
 * interpretAnswer — Simple keyword/sentiment analysis to convert a free-text
 * answer into a 0-100 slider value based on the question's low/high labels.
 */
function interpretAnswer(msg, question) {
  // Strong directional keywords
  const highSignals = ['yes', 'definitely', 'absolutely', 'always', 'love', 'prefer', 'very much', 
    'strong', 'high', 'max', '100', 'totally', 'for sure', 'hell yeah', 'exactly', 'all the way'];
  const lowSignals = ['no', 'never', 'not really', 'hate', 'avoid', 'minimal', 'low', 'none',
    'not at all', 'nah', 'nope', 'rarely', 'barely'];
  const midSignals = ['sometimes', 'depends', 'balanced', 'mix', 'both', 'moderate', 'middle',
    'it depends', 'kinda', 'sort of', 'a bit'];

  // Check for explicit numbers (e.g., "like a 7 out of 10")
  const numMatch = msg.match(/\b(\d{1,3})\s*(out of|\/)\s*(10|100)\b/);
  if (numMatch) {
    const val = parseInt(numMatch[1]);
    const max = parseInt(numMatch[3]);
    return Math.min(100, Math.max(0, Math.round((val / max) * 100)));
  }

  // Simple number on its own
  const simpleNum = msg.match(/^(\d{1,3})$/);
  if (simpleNum) {
    const n = parseInt(simpleNum[1]);
    if (n <= 10) return n * 10;
    if (n <= 100) return n;
  }

  // Check for low/high label references in the answer
  const lowLabel = (question.lowLabel || '').toLowerCase();
  const highLabel = (question.highLabel || '').toLowerCase();
  if (lowLabel && msg.includes(lowLabel.substring(0, Math.min(lowLabel.length, 8)))) return 20;
  if (highLabel && msg.includes(highLabel.substring(0, Math.min(highLabel.length, 8)))) return 80;

  // Keyword matching
  let score = null;
  for (const kw of highSignals) {
    if (msg.includes(kw)) { score = (score || 0) + 75; break; }
  }
  for (const kw of lowSignals) {
    if (msg.includes(kw)) { score = score !== null ? Math.min(score, 25) : 25; break; }
  }
  for (const kw of midSignals) {
    if (msg.includes(kw)) { score = score !== null ? score : 50; break; }
  }

  // If we got a keyword match, return it (clamped)
  if (score !== null) return Math.min(100, Math.max(0, score));

  // If the answer is substantive (>15 chars), default to moderate (they engaged)
  if (msg.length > 15) return 55;

  return null; // Can't interpret
}
