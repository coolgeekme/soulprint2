...
export async function handleMemoryCleanupScan(request) {
  ...
  // Persist the staleness signal instead of only reporting it: a flagged memory
  // becomes "don't trust yet" (review_state), never silently deleted. Editing or
  // re-verifying a memory clears the flag (see handleUpdateMemory).
  const staleIds = suggestions
    .filter((s) => s.type === 'stale')
    .flatMap((s) => (s.memories || []).map((m) => m.id));
  if (staleIds.length) {
    await db.collection('user_memories').updateMany(
      { user_id: user.id, id: { $in: staleIds } },
      { $set: { review_state: 'needs_review', flagged_at: new Date() } }
    );
    console.log(`[MemoryCleanup] Flagged ${staleIds.length} memories as needs_review`);
  }

  // Memories that survived the scan clean are considered re-verified.
  const staleSet = new Set(staleIds);
  const cleanIds = memories.filter((m) => !staleSet.has(m.id)).map((m) => m.id);
  if (cleanIds.length) {
    await db.collection('user_memories').updateMany(
      { user_id: user.id, id: { $in: cleanIds }, review_state: 'needs_review' },
      { $set: { review_state: 'active', last_verified_at: new Date(), flagged_at: null } }
    );
  }
