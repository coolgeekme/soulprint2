...
const SUPPORT_EMAIL = 'team@archeforge.com';
...
You are Ace, the SoulPrint AI Support Agent — an expert assistant embedded directly in the SoulPrint Engine platform. You help users with ANYTHING related to the platform — from chat features, media generation, voice chat, account issues, to advanced prompting techniques. Your name is Ace.
...
## YOUR BEHAVIOR
- Your name is Ace. Introduce yourself as Ace if the user asks who you are.
...
- If you can't solve it, offer escalation to human support — tell them to click "Escalate to support" below the chat
...
export async function handleSupportEscalate(request) {
  try {
    ...
    const ticket = {
      ...
      subject: subject || 'User escalation from Ace (AI Support)',
      description: description,
      status: 'new',
      source: 'ace_escalation',
    ...
    // Auto-lookup user data to enrich the ticket for support team
    try {
        ...
        await db.collection('support_tickets').updateOne({ id: ticketId }, {
          $set: {
            user_data: {
              ...
              name: userData.display_name || userData.name || userData.email,
              role: userData.role,
    ...
    try {
      const botConvoHtml = sessionMessages.length > 0
        ? `<h3 style="color:#f97316;margin-top:20px;">Ace Conversation History</h3>
           <div style="background:#1a1a2e;border-radius:8px;padding:12px;margin-top:8px;">
           ${sessionMessages.map(m => `<p style="margin:6px 0;color:${m.role === 'user' ? '#60a5fa' : '#a78bfa'};font-size:13px;"><strong>${m.role === 'user' ? '👤 User' : '🤖 Ace'}:</strong> ${(m.content || '').substring(0, 500)}</p>`).join('')}
           </div>`
        : '';

      await sendEmail({
        to: SUPPORT_EMAIL,
        subject: `🎫 New Support Ticket: ${subject || 'User Escalation'} [#${ticketId.slice(0, 8)}]`,
        html: `
          <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0f0f1a;color:#e5e7eb;padding:24px;border-radius:12px;">
            <h2 style="color:#f97316;margin-top:0;">New Support Ticket from Ace</h2>
            <table style="width:100%;border-collapse:collapse;margin:16px 0;">
              <tr><td style="padding:6px 12px;color:#9ca3af;font-size:13px;">Ticket ID</td><td style="padding:6px 12px;color:#fff;font-size:13px;font-family:monospace;">${ticketId}</td></tr>
              <tr><td style="padding:6px 12px;color:#9ca3af;font-size:13px;">User</td><td style="padding:6px 12px;color:#fff;font-size:13px;">${user.display_name || user.email} (${user.email})</td></tr>
              <tr><td style="padding:6px 12px;color:#9ca3af;font-size:13px;">User ID</td><td style="padding:6px 12px;color:#fff;font-size:13px;font-family:monospace;">${user.id}</td></tr>
              ${conversationId ? `<tr><td style="padding:6px 12px;color:#9ca3af;font-size:13px;">Conversation ID</td><td style="padding:6px 12px;color:#fff;font-size:13px;font-family:monospace;">${conversationId}</td></tr>` : ''}
              <tr><td style="padding:6px 12px;color:#9ca3af;font-size:13px;">Subject</td><td style="padding:6px 12px;color:#fff;font-size:13px;">${subject || 'User Escalation'}</td></tr>
            </table>
            <h3 style="color:#60a5fa;margin-top:20px;">Issue Description</h3>
            <div style="background:#1a1a2e;border-radius:8px;padding:16px;margin-top:8px;">
              <p style="color:#e5e7eb;font-size:14px;white-space:pre-wrap;margin:0;">${description}</p>
            </div>
            ${botConvoHtml}
            <div style="margin-top:24px;text-align:center;">
              <a href="${process.env.NEXT_PUBLIC_BASE_URL || 'https://soulprint-engine.preview.emergentagent.com'}/admin" style="display:inline-block;padding:12px 24px;background:#f97316;color:#fff;text-decoration:none;border-radius:8px;font-weight:600;">Review in Admin Dashboard →</a>
            </div>
            <p style="color:#6b7280;font-size:11px;margin-top:20px;text-align:center;">This ticket was auto-generated when a user escalated from Ace (AI Support Agent)</p>
          </div>`,
      });
    } catch (emailErr) {
      console.warn('[SupportEscalate] Email send failed:', emailErr.message);
    }
