/**
 * ═══════════════════════════════════════════════════════════════════════════
 * ACCESS ENFORCEMENT ENGINE — Phase 5
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * Determines a user's effective access level based on:
 *   1. Registration date (cohort classification)
 *   2. Grace period status
 *   3. Assessment completion (for new trial users)
 *   4. Current subscription plan
 *   5. Admin/superadmin exemption
 * 
 * Cohorts:
 *   - 'og'         → Registered March 1–31, 2026 (Alpha Users). Full access until May 31, 2026.
 *   - 'early'      → Registered April 1–30, 2026. Full access until May 14, 2026.
 *   - 'new_trial'  → Registered after May 1, 2026. Base-plan trial until assessment
 *                     complete OR any plan limit hit.
 *   - 'subscribed' → Has an active paid subscription (Base or Power).
 *   - 'admin'      → Admin/superadmin, always exempt from enforcement.
 * 
 * After grace period expires:
 *   - Soft block: Premium models visible but greyed out with "Upgrade to unlock"
 *   - Old conversations using premium models show fallback notice
 *   - Daily message limit enforced for Free plan (50/day)
 * ═══════════════════════════════════════════════════════════════════════════
 */

import { getDb } from '@/lib/mongodb';

// ── Key Dates ───────────────────────────────────────────────────────────────
const PRICING_LAUNCH_DATE = new Date('2026-05-01T00:00:00Z');
const ALPHA_START_DATE = new Date('2026-03-01T00:00:00Z');  // Alpha users start
const OG_CUTOFF_DATE = new Date('2026-04-01T00:00:00Z');    // End of Alpha, start of Early
const OG_GRACE_END = new Date('2026-05-31T23:59:59Z');      // Alpha users: full access until May 31
const EARLY_GRACE_END = new Date('2026-05-14T23:59:59Z');   // April users: full access until May 14

// ── Plan Feature Defaults (used for trial equivalents) ──────────────────────
const BASE_TRIAL_FEATURES = {
  chat_model_tier: 'all',
  premium_chat: true,
  premium_chat_msgs_per_month: 50,
  standard_msgs_per_day: null, // unlimited during trial
  images_per_month: 50,
  image_watermark: false,
  videos_per_month: 1,
  video_duration_sec: 5,
  video_resolution: '720p',
  pdfs_per_month: 25,
  voice_chat: true,
  voice_minutes_per_month: 30,
  file_analysis_advanced: true,
  conversation_search: true,
};

const UNRESTRICTED_FEATURES = {
  chat_model_tier: 'all',
  premium_chat: true,
  premium_chat_unlimited: true,
  standard_msgs_per_day: null,
  images_per_month: null,
  image_watermark: false,
  videos_per_month: null,
  video_duration_sec: null,
  video_resolution: 'all',
  pdfs_per_month: null,
  voice_chat: true,
  voice_unlimited: true,
  file_analysis_advanced: true,
  conversation_search: true,
};

/**
 * Get a user's enforcement status — the single source of truth for access control.
 * 
 * @param {string} userId - The user's UUID
 * @returns {Object} Enforcement status object
 */
export async function getUserEnforcementStatus(userId) {
  const db = await getDb();
  const now = new Date();
  
  // ── Fetch user data ───────────────────────────────────────────────────────
  const user = await db.collection('users').findOne({ id: userId });
  if (!user) {
    return {
      cohort: 'unknown',
      enforcement_active: true,
      effective_plan: 'free',
      effective_features: null,
      grace_expires_at: null,
      days_remaining: 0,
      show_countdown: false,
      assessment_complete: false,
      is_trial: false,
      trial_limit_hit: false,
      error: 'User not found',
    };
  }

  // ── Admin exemption ───────────────────────────────────────────────────────
  if (user.role === 'admin' || user.role === 'superadmin') {
    return {
      cohort: 'admin',
      enforcement_active: false,
      effective_plan: 'power',
      effective_features: UNRESTRICTED_FEATURES,
      grace_expires_at: null,
      days_remaining: null,
      show_countdown: false,
      assessment_complete: true,
      is_trial: false,
      trial_limit_hit: false,
    };
  }

  // ── Team member exemption (@archeforge.com) ────────────────────────────────
  // ArcheForge team members get unlimited access regardless of subscription
  if (user.email && user.email.toLowerCase().endsWith('@archeforge.com')) {
    return {
      cohort: 'team',
      enforcement_active: false,
      effective_plan: 'power',
      effective_features: UNRESTRICTED_FEATURES,
      grace_expires_at: null,
      days_remaining: null,
      show_countdown: false,
      assessment_complete: true,
      is_trial: false,
      trial_limit_hit: false,
    };
  }

  // ── Fetch subscription & profile ──────────────────────────────────────────
  const subscription = await db.collection('user_subscriptions').findOne({ user_id: userId });
  const profile = await db.collection('profiles').findOne({ user_id: userId });
  
  const planId = subscription?.plan_id || 'free';
  const registeredAt = user.created_at ? new Date(user.created_at) : now;
  const assessmentComplete = profile?.assessment_complete === true;

  // ── Active paid subscriber — no enforcement needed ────────────────────────
  if (planId !== 'free' && subscription?.status === 'active') {
    const plan = await db.collection('subscription_plans').findOne({ id: planId });
    return {
      cohort: 'subscribed',
      enforcement_active: false,
      effective_plan: planId,
      effective_features: plan?.features || UNRESTRICTED_FEATURES,
      grace_expires_at: null,
      days_remaining: null,
      show_countdown: false,
      assessment_complete: assessmentComplete,
      is_trial: false,
      trial_limit_hit: false,
    };
  }

  // ── Pre-launch: No enforcement active yet ─────────────────────────────────
  if (now < PRICING_LAUNCH_DATE) {
    return {
      cohort: classifyCohort(registeredAt),
      enforcement_active: false,
      effective_plan: 'power_equivalent',
      effective_features: UNRESTRICTED_FEATURES,
      grace_expires_at: getGraceEndDate(registeredAt),
      days_remaining: getDaysRemaining(getGraceEndDate(registeredAt), now),
      show_countdown: false, // Don't show countdown before launch
      assessment_complete: assessmentComplete,
      is_trial: false,
      trial_limit_hit: false,
    };
  }

  // ── Post-launch logic ─────────────────────────────────────────────────────
  const cohort = classifyCohort(registeredAt);
  const graceEnd = getGraceEndDate(registeredAt);
  const daysRemaining = getDaysRemaining(graceEnd, now);
  const graceActive = graceEnd && now <= graceEnd;

  // ── OG / Early users still in grace period ────────────────────────────────
  if ((cohort === 'og' || cohort === 'early') && graceActive) {
    return {
      cohort,
      enforcement_active: false,
      effective_plan: 'power_equivalent',
      effective_features: UNRESTRICTED_FEATURES,
      grace_expires_at: graceEnd,
      days_remaining: daysRemaining,
      show_countdown: daysRemaining <= 7,
      assessment_complete: assessmentComplete,
      is_trial: false,
      trial_limit_hit: false,
    };
  }

  // ── New user trial (post May 1, assessment not complete) ──────────────────
  if (cohort === 'new_user' && !assessmentComplete) {
    // Check if they've hit any trial limits
    const trialLimitHit = await checkTrialLimitsHit(userId, db);
    
    if (!trialLimitHit) {
      return {
        cohort: 'new_trial',
        enforcement_active: false, // Trial is active, no enforcement yet
        effective_plan: 'base_trial',
        effective_features: BASE_TRIAL_FEATURES,
        grace_expires_at: null, // No fixed date — ends on assessment/limit
        days_remaining: null,
        show_countdown: false,
        assessment_complete: false,
        is_trial: true,
        trial_limit_hit: false,
      };
    }
    
    // Trial limit hit — must choose a plan
    return {
      cohort: 'new_trial',
      enforcement_active: true,
      effective_plan: 'free',
      effective_features: await getFreePlanFeatures(db),
      grace_expires_at: null,
      days_remaining: 0,
      show_countdown: false,
      assessment_complete: false,
      is_trial: false,
      trial_limit_hit: true,
      choose_plan_prompt: true,
    };
  }

  // ── EXPLICIT FREE PLAN SELECTION ──────────────────────────────────────────
  // If user explicitly selected Free plan (not just defaulted to it), don't prompt
  if (planId === 'free' && subscription?.admin_override_reason === 'user_selected_free_plan') {
    return {
      cohort: cohort || 'free_user',
      enforcement_active: false,
      effective_plan: 'free',
      effective_features: await getFreePlanFeatures(db),
      grace_expires_at: null,
      days_remaining: 0,
      show_countdown: false,
      assessment_complete: assessmentComplete,
      is_trial: false,
      trial_limit_hit: false,
      choose_plan_prompt: false, // User explicitly chose Free - don't prompt
      user_selected_free: true,
    };
  }

  // ── New user post-assessment: reward window ONLY if completed within 7 days of signup ─
  if (cohort === 'new_user' && assessmentComplete && planId === 'free') {
    const profile = await db.collection('profiles').findOne({ user_id: userId });
    const completedAt = profile?.assessment_completed_at;
    const userRecord = await db.collection('users').findOne({ $or: [{ id: userId }, { user_id: userId }] });
    const registeredAt = userRecord?.created_at;
    
    // Check if assessment was completed within 7 days of registration
    const completedWithin7Days = completedAt && registeredAt && 
      ((new Date(completedAt) - new Date(registeredAt)) / (1000 * 60 * 60 * 24)) <= 7;
    
    if (completedWithin7Days) {
      const rewardEndDate = new Date(completedAt);
      rewardEndDate.setDate(rewardEndDate.getDate() + 7);
      const now = new Date();
      const rewardDaysRemaining = Math.max(0, Math.ceil((rewardEndDate - now) / (1000 * 60 * 60 * 24)));
      
      if (now < rewardEndDate) {
        // Still within 7-day reward window — keep full Base-trial access
        return {
          cohort: 'post_assessment_reward',
          enforcement_active: false,
          effective_plan: 'base_trial',
          effective_features: BASE_TRIAL_FEATURES,
          grace_expires_at: rewardEndDate,
          days_remaining: rewardDaysRemaining,
          show_countdown: rewardDaysRemaining <= 3,
          assessment_complete: true,
          is_trial: true,
          trial_limit_hit: false,
          reward_window: true,
          reward_message: `Your SoulPrint is active! ${rewardDaysRemaining} day${rewardDaysRemaining !== 1 ? 's' : ''} of full access remaining. Choose a plan to keep it going.`,
        };
      }
    }
    
    // Assessment completed after 7 days, or reward window expired — drop to Free, prompt plan selection
    return {
      cohort: 'post_trial',
      enforcement_active: true,
      effective_plan: 'free',
      effective_features: await getFreePlanFeatures(db),
      grace_expires_at: null,
      days_remaining: 0,
      show_countdown: false,
      assessment_complete: true,
      is_trial: false,
      trial_limit_hit: false,
      choose_plan_prompt: true,
    };
  }

  // ── Grace expired for OG/Early users ──────────────────────────────────────
  if ((cohort === 'og' || cohort === 'early') && !graceActive) {
    return {
      cohort,
      enforcement_active: true,
      effective_plan: 'free',
      effective_features: await getFreePlanFeatures(db),
      grace_expires_at: graceEnd,
      days_remaining: 0,
      show_countdown: false,
      assessment_complete: assessmentComplete,
      is_trial: false,
      trial_limit_hit: false,
      choose_plan_prompt: true,
      grace_expired: true,
      grace_expired_message: cohort === 'early'
        ? 'Your early access grace period ended on May 14. Choose a plan to keep your premium features.'
        : 'Your alpha grace period ended on May 31. Choose a plan to continue with premium features.',
    };
  }

  // ── Fallback: enforce free plan ───────────────────────────────────────────
  return {
    cohort: cohort || 'unknown',
    enforcement_active: true,
    effective_plan: 'free',
    effective_features: await getFreePlanFeatures(db),
    grace_expires_at: graceEnd,
    days_remaining: 0,
    show_countdown: false,
    assessment_complete: assessmentComplete,
    is_trial: false,
    trial_limit_hit: false,
  };
}

// ── Upgrade nudge metadata for contextual conversion ──────────────────────────
const UPGRADE_NUDGE_META = {
  premium_model: {
    emoji: '🧠',
    title: 'Unlock Premium AI Models',
    value_props: ['GPT-4o, Claude 3.5, Gemini Pro', 'Deeper reasoning & creativity', 'Faster responses'],
    cta: 'Upgrade to Base',
    price_hint: 'Starting at $9.99/mo',
  },
  standard_message: {
    emoji: '💬',
    title: 'Keep the Conversation Going',
    value_props: ['Unlimited daily messages', 'No interruptions mid-conversation', 'Priority response times'],
    cta: 'Upgrade Plan',
    price_hint: 'Starting at $9.99/mo',
  },
  image_generation: {
    emoji: '🎨',
    title: 'Create Unlimited Images',
    value_props: ['HD image generation with no watermark', 'All AI models (DALL-E, Flux, Kie.ai)', 'Edit & iterate on your creations'],
    cta: 'Upgrade to Create More',
    price_hint: 'Starting at $9.99/mo',
  },
  video_generation: {
    emoji: '🎬',
    title: 'Unlock AI Video Creation',
    value_props: ['Generate cinematic AI videos', 'Multiple models (Kling, Veo, Runway)', 'Up to 10s clips at 1080p'],
    cta: 'Upgrade for Video',
    price_hint: 'Starting at $9.99/mo',
  },
  voice_chat: {
    emoji: '🎙️',
    title: 'Talk to Your AI — Literally',
    value_props: ['Real-time voice conversations', 'Natural AI voice responses', 'Hands-free interaction'],
    cta: 'Upgrade for Voice',
    price_hint: 'Starting at $9.99/mo',
  },
  pdf_generation: {
    emoji: '📄',
    title: 'Generate More Documents',
    value_props: ['Create professional PDFs', 'Export chat summaries', 'Unlimited document generation'],
    cta: 'Upgrade Plan',
    price_hint: 'Starting at $9.99/mo',
  },
};

// ── Premium Preview Allocations (one-time per user) ──────────────────────────
const PREMIUM_PREVIEW_ALLOCATIONS = {
  image_generation: 2,   // 2 free HD images
  video_generation: 1,   // 1 free video
  voice_chat: 1,         // 1 free voice session
};

/**
 * Check if a specific action is allowed for the user.
 * This is the primary enforcement gate — called before premium model access,
 * image/video generation, voice, and PDF generation.
 * 
 * Enhanced with:
 *   - Usage warnings at 80% threshold
 *   - Premium preview credits for blocked actions
 *   - Rich upgrade nudge metadata for contextual conversion
 * 
 * @param {string} userId
 * @param {string} action - One of: 'premium_model', 'standard_message', 'image_generation',
 *                          'video_generation', 'voice_chat', 'pdf_generation', 'conversation_search'
 * @param {Object} context - Additional context (e.g., { model: 'gpt-5.2' })
 * @returns {Object} { allowed: true, warning? } or { allowed: false, reason, upgrade_nudge, preview_available? }
 */
export async function checkActionAllowed(userId, action, context = {}) {
  const status = await getUserEnforcementStatus(userId);
  
  // No enforcement — everything allowed (but still check for soft warnings)
  if (!status.enforcement_active) {
    return { allowed: true, status };
  }

  const features = status.effective_features;
  if (!features) {
    return { allowed: false, reason: 'Unable to determine plan features', status };
  }

  const db = await getDb();
  const nudgeMeta = UPGRADE_NUDGE_META[action] || {};

  switch (action) {
    case 'premium_model': {
      if (!features.premium_chat) {
        const preview = await _checkPreviewAvailable(userId, action, db);
        return {
          allowed: false,
          reason: 'Premium models require a Base or Power subscription',
          upgrade_url: '/pricing',
          alternatives: await getStandardModels(),
          soft_block: true,
          upgrade_nudge: nudgeMeta,
          preview_available: preview.available,
          preview_remaining: preview.remaining,
          status,
        };
      }
      // Check monthly premium message limit
      if (features.premium_chat_msgs_per_month && !features.premium_chat_unlimited) {
        const used = await getPremiumMsgsUsedThisMonth(userId, db);
        const limit = features.premium_chat_msgs_per_month;
        if (used >= limit) {
          return {
            allowed: false,
            reason: `You've used all ${limit} premium messages this month`,
            upgrade_url: '/pricing',
            add_on_available: true,
            add_on_type: 'premium_messages',
            used,
            limit,
            soft_block: true,
            upgrade_nudge: nudgeMeta,
            status,
          };
        }
        // ── 80% usage warning ──
        const warning = _getUsageWarning(used, limit, 'premium messages', 'monthly');
        if (warning) return { allowed: true, warning, status };
      }
      return { allowed: true, status };
    }

    case 'standard_message': {
      if (features.standard_msgs_per_day) {
        const used = await getStandardMsgsUsedToday(userId, db);
        const limit = features.standard_msgs_per_day;
        if (used >= limit) {
          return {
            allowed: false,
            reason: `You've reached your daily limit of ${limit} messages. Upgrade to keep chatting, or come back tomorrow.`,
            upgrade_url: '/pricing',
            used,
            limit,
            resets_at: getNextMidnightUTC(),
            soft_block: true,
            upgrade_nudge: nudgeMeta,
            status,
          };
        }
        // ── 80% usage warning ──
        const warning = _getUsageWarning(used, limit, 'messages', 'today');
        if (warning) return { allowed: true, warning, status };
      }
      return { allowed: true, status };
    }

    case 'image_generation': {
      if (features.images_per_month === 0) {
        const preview = await _checkPreviewAvailable(userId, action, db);
        return {
          allowed: false,
          reason: 'Image generation is not available on your current plan',
          upgrade_url: '/pricing',
          soft_block: true,
          upgrade_nudge: nudgeMeta,
          preview_available: preview.available,
          preview_remaining: preview.remaining,
          status,
        };
      }
      if (features.images_per_month !== null) {
        const used = await getImagesUsedThisMonth(userId, db);
        const limit = features.images_per_month;
        if (used >= limit) {
          const preview = await _checkPreviewAvailable(userId, action, db);
          return {
            allowed: false,
            reason: `You've used all ${limit} images this month`,
            upgrade_url: '/pricing',
            add_on_available: true,
            add_on_type: 'media_credits',
            used,
            limit,
            soft_block: true,
            upgrade_nudge: nudgeMeta,
            preview_available: preview.available,
            preview_remaining: preview.remaining,
            status,
          };
        }
        // ── 80% usage warning ──
        const warning = _getUsageWarning(used, limit, 'images', 'this month');
        if (warning) return { allowed: true, warning, status };
      }
      // Check hourly rate limit
      if (features.image_rate_limit_per_hour) {
        const usedThisHour = await getImagesUsedThisHour(userId, db);
        if (usedThisHour >= features.image_rate_limit_per_hour) {
          return {
            allowed: false,
            reason: `Rate limit: max ${features.image_rate_limit_per_hour} images per hour. Try again soon.`,
            rate_limited: true,
            status,
          };
        }
      }
      return { allowed: true, watermark: features.image_watermark || false, status };
    }

    case 'video_generation': {
      if (features.videos_per_month === 0 || features.videos_per_month === null && features.chat_model_tier === 'standard') {
        const preview = await _checkPreviewAvailable(userId, action, db);
        return {
          allowed: false,
          reason: 'Video generation is not available on the Free plan',
          upgrade_url: '/pricing',
          add_on_available: true,
          add_on_type: 'media_credits',
          soft_block: true,
          upgrade_nudge: nudgeMeta,
          preview_available: preview.available,
          preview_remaining: preview.remaining,
          status,
        };
      }
      if (features.videos_per_month !== null) {
        const used = await getVideosUsedThisMonth(userId, db);
        const limit = features.videos_per_month;
        if (used >= limit) {
          const preview = await _checkPreviewAvailable(userId, action, db);
          return {
            allowed: false,
            reason: `You've used your ${limit} video${limit > 1 ? 's' : ''} this month`,
            upgrade_url: '/pricing',
            add_on_available: true,
            add_on_type: 'media_credits',
            used,
            limit,
            soft_block: true,
            upgrade_nudge: nudgeMeta,
            preview_available: preview.available,
            preview_remaining: preview.remaining,
            status,
          };
        }
        // ── 80% usage warning ──
        const warning = _getUsageWarning(used, limit, 'videos', 'this month');
        if (warning) return { allowed: true, warning, status };
      }
      return { allowed: true, watermark: features.video_watermark || false, status };
    }

    case 'voice_chat': {
      if (!features.voice_chat) {
        const preview = await _checkPreviewAvailable(userId, action, db);
        return {
          allowed: false,
          reason: 'Voice chat requires a Base or Power subscription',
          upgrade_url: '/pricing',
          soft_block: true,
          upgrade_nudge: nudgeMeta,
          preview_available: preview.available,
          preview_remaining: preview.remaining,
          status,
        };
      }
      if (features.voice_minutes_per_month && !features.voice_unlimited) {
        const used = await getVoiceMinutesUsedThisMonth(userId, db);
        const limit = features.voice_minutes_per_month;
        if (used >= limit) {
          return {
            allowed: false,
            reason: `You've used all ${limit} voice minutes this month`,
            upgrade_url: '/pricing',
            used,
            limit,
            soft_block: true,
            upgrade_nudge: nudgeMeta,
            status,
          };
        }
        // ── 80% usage warning ──
        const warning = _getUsageWarning(used, limit, 'voice minutes', 'this month');
        if (warning) return { allowed: true, warning, status };
      }
      return { allowed: true, status };
    }

    case 'pdf_generation': {
      if (features.pdfs_per_month !== null && features.pdfs_per_month !== undefined) {
        const used = await getPdfsUsedThisMonth(userId, db);
        const limit = features.pdfs_per_month;
        if (used >= limit) {
          return {
            allowed: false,
            reason: `You've used all ${limit} PDF generations this month`,
            upgrade_url: '/pricing',
            used,
            limit,
            soft_block: true,
            upgrade_nudge: nudgeMeta,
            status,
          };
        }
        // ── 80% usage warning ──
        const warning = _getUsageWarning(used, limit, 'PDFs', 'this month');
        if (warning) return { allowed: true, warning, status };
      }
      return { allowed: true, status };
    }

    case 'conversation_search': {
      if (!features.conversation_search) {
        return {
          allowed: false,
          reason: 'Conversation search requires a Base or Power subscription',
          upgrade_url: '/pricing',
          soft_block: true,
          upgrade_nudge: nudgeMeta,
          status,
        };
      }
      return { allowed: true, status };
    }

    default:
      return { allowed: true, status };
  }
}

// ── Usage Warning Helper (80% threshold) ────────────────────────────────────
function _getUsageWarning(used, limit, resourceName, period) {
  if (!limit || limit <= 0) return null;
  const percentage = Math.round((used / limit) * 100);
  if (percentage < 80) return null;
  
  const remaining = limit - used;
  return {
    approaching_limit: true,
    used,
    limit,
    remaining,
    percentage,
    resource: resourceName,
    period,
    message: remaining <= 1
      ? `⚡ Last ${resourceName.slice(0, -1) || resourceName} — you've used ${used} of ${limit} ${resourceName} ${period}`
      : `You've used ${used} of ${limit} ${resourceName} ${period} (${remaining} remaining)`,
  };
}

// ── Premium Preview System ──────────────────────────────────────────────────
async function _checkPreviewAvailable(userId, action, db) {
  const allocation = PREMIUM_PREVIEW_ALLOCATIONS[action];
  if (!allocation) return { available: false, remaining: 0 };
  
  const previewDoc = await db.collection('premium_previews').findOne({
    user_id: userId,
    action,
  });
  
  const usedCount = previewDoc?.used_count || 0;
  const remaining = Math.max(0, allocation - usedCount);
  return { available: remaining > 0, remaining };
}

/**
 * Use a premium preview credit for a specific action.
 * Returns { success: true, remaining } or { success: false, reason }.
 */
export async function usePremiumPreview(userId, action) {
  const db = await getDb();
  const allocation = PREMIUM_PREVIEW_ALLOCATIONS[action];
  if (!allocation) return { success: false, reason: 'No preview available for this action' };
  
  const result = await db.collection('premium_previews').findOneAndUpdate(
    { user_id: userId, action },
    { 
      $inc: { used_count: 1 },
      $set: { last_used_at: new Date() },
      $setOnInsert: { user_id: userId, action, created_at: new Date() },
    },
    { upsert: true, returnDocument: 'after' }
  );
  
  const usedCount = result?.used_count || result?.value?.used_count || 1;
  if (usedCount > allocation) {
    // Rolled past allocation — undo
    await db.collection('premium_previews').updateOne(
      { user_id: userId, action },
      { $inc: { used_count: -1 } }
    );
    return { success: false, reason: 'No preview credits remaining' };
  }
  
  const remaining = allocation - usedCount;
  console.log(`[PremiumPreview] User ${userId} used ${action} preview (${remaining} remaining)`);
  return { success: true, remaining, total: allocation };
}

/**
 * Record usage for a specific action (call AFTER the action succeeds).
 * 
 * @param {string} userId
 * @param {string} action - 'standard_message', 'premium_message', 'image', 'video', 'pdf', 'voice_minute'
 * @param {Object} metadata - Additional data (e.g., { model: 'gpt-5.2', credits_used: 5 })
 */
export async function recordUsage(userId, action, metadata = {}) {
  const db = await getDb();
  const now = new Date();
  const todayStr = now.toISOString().split('T')[0]; // YYYY-MM-DD
  const monthStr = todayStr.substring(0, 7); // YYYY-MM
  const hourStr = now.toISOString().substring(0, 13); // YYYY-MM-DDTHH

  const usageDoc = {
    user_id: userId,
    action,
    timestamp: now,
    date: todayStr,
    month: monthStr,
    hour: hourStr,
    ...metadata,
  };

  await db.collection('usage_log').insertOne(usageDoc);

  // Update aggregated counters for fast lookups
  const counterKey = `${userId}:${action}`;
  let newCount = 0;
  let counterPeriod = '';
  
  if (action === 'standard_message') {
    const result = await db.collection('usage_counters').findOneAndUpdate(
      { key: `${counterKey}:daily:${todayStr}` },
      { $inc: { count: 1 }, $set: { user_id: userId, action, period: todayStr, updated_at: now } },
      { upsert: true, returnDocument: 'after' }
    );
    newCount = result?.count || result?.value?.count || 0;
    counterPeriod = 'daily';
  }
  
  if (action === 'premium_message') {
    const result = await db.collection('usage_counters').findOneAndUpdate(
      { key: `${counterKey}:monthly:${monthStr}` },
      { $inc: { count: 1 }, $set: { user_id: userId, action, period: monthStr, updated_at: now } },
      { upsert: true, returnDocument: 'after' }
    );
    newCount = result?.count || result?.value?.count || 0;
    counterPeriod = 'monthly';
  }

  if (action === 'image') {
    const result = await db.collection('usage_counters').findOneAndUpdate(
      { key: `${counterKey}:monthly:${monthStr}` },
      { $inc: { count: 1 }, $set: { user_id: userId, action, period: monthStr, updated_at: now } },
      { upsert: true, returnDocument: 'after' }
    );
    newCount = result?.count || result?.value?.count || 0;
    counterPeriod = 'monthly';
    await db.collection('usage_counters').updateOne(
      { key: `${counterKey}:hourly:${hourStr}` },
      { $inc: { count: 1 }, $set: { user_id: userId, action, period: hourStr, updated_at: now } },
      { upsert: true }
    );
  }

  if (action === 'video') {
    const result = await db.collection('usage_counters').findOneAndUpdate(
      { key: `${counterKey}:monthly:${monthStr}` },
      { $inc: { count: 1 }, $set: { user_id: userId, action, period: monthStr, updated_at: now } },
      { upsert: true, returnDocument: 'after' }
    );
    newCount = result?.count || result?.value?.count || 0;
    counterPeriod = 'monthly';
  }

  if (action === 'pdf') {
    const result = await db.collection('usage_counters').findOneAndUpdate(
      { key: `${counterKey}:monthly:${monthStr}` },
      { $inc: { count: 1 }, $set: { user_id: userId, action, period: monthStr, updated_at: now } },
      { upsert: true, returnDocument: 'after' }
    );
    newCount = result?.count || result?.value?.count || 0;
    counterPeriod = 'monthly';
  }

  if (action === 'voice_minute') {
    await db.collection('usage_counters').updateOne(
      { key: `${counterKey}:monthly:${monthStr}` },
      { $inc: { count: 1 }, $set: { user_id: userId, action, period: monthStr, updated_at: now } },
      { upsert: true }
    );
  }

  // Fire-and-forget: Check if we need to send a usage alert email
  if (newCount > 0 && counterPeriod) {
    _checkUsageAlert(userId, action, newCount, db).catch(() => {});
  }
}

// Internal helper to check usage limits and trigger alert emails
async function _checkUsageAlert(userId, action, currentCount, db) {
  try {
    const status = await getUserEnforcementStatus(userId);
    const features = status.effective_features;
    let limit = 0;
    let usageType = '';
    
    if (action === 'standard_message') {
      limit = features?.messages_per_day || 0;
      usageType = 'messages';
    } else if (action === 'image') {
      limit = features?.images_per_month || 0;
      usageType = 'images';
    } else if (action === 'video') {
      limit = features?.videos_per_month || 0;
      usageType = 'videos';
    } else if (action === 'pdf') {
      limit = features?.pdfs_per_month || 0;
      usageType = 'pdfs';
    }
    
    if (!limit || limit <= 0 || !usageType) return;
    
    const { checkAndSendUsageAlert } = await import('@/lib/handlers/usage-alerts');
    await checkAndSendUsageAlert(userId, usageType, currentCount, limit);
  } catch (err) {
    // Non-critical, silently fail
  }
}

/**
 * Get the user's current usage summary for the billing dashboard.
 */
export async function getUserUsageSummary(userId) {
  const db = await getDb();
  const now = new Date();
  const todayStr = now.toISOString().split('T')[0];
  const monthStr = todayStr.substring(0, 7);
  const status = await getUserEnforcementStatus(userId);
  const features = status.effective_features;

  const [stdMsgsToday, premiumMsgsMonth, imagesMonth, videosMonth, pdfsMonth, voiceMonth] = await Promise.all([
    getStandardMsgsUsedToday(userId, db),
    getPremiumMsgsUsedThisMonth(userId, db),
    getImagesUsedThisMonth(userId, db),
    getVideosUsedThisMonth(userId, db),
    getPdfsUsedThisMonth(userId, db),
    getVoiceMinutesUsedThisMonth(userId, db),
  ]);

  // Get media credits balance
  const creditsDoc = await db.collection('user_credits').findOne({ user_id: userId });
  const mediaCredits = creditsDoc?.media_credits || 0;
  const premiumMessagesBalance = creditsDoc?.premium_messages || 0;

  return {
    plan: status.effective_plan,
    cohort: status.cohort,
    enforcement_active: status.enforcement_active,
    grace_expires_at: status.grace_expires_at,
    days_remaining: status.days_remaining,
    show_countdown: status.show_countdown,
    is_trial: status.is_trial,
    media_credits_balance: mediaCredits,
    premium_messages_balance: premiumMessagesBalance,
    usage: {
      standard_messages: {
        used: stdMsgsToday,
        limit: features?.standard_msgs_per_day || null,
        period: 'daily',
        resets_at: features?.standard_msgs_per_day ? getNextMidnightUTC() : null,
      },
      premium_messages: {
        used: premiumMsgsMonth,
        limit: features?.premium_chat_unlimited ? null : (features?.premium_chat_msgs_per_month || 0),
        period: 'monthly',
      },
      images: {
        used: imagesMonth,
        limit: features?.images_per_month,
        period: 'monthly',
        watermark: features?.image_watermark || false,
      },
      videos: {
        used: videosMonth,
        limit: features?.videos_per_month,
        period: 'monthly',
      },
      pdfs: {
        used: pdfsMonth,
        limit: features?.pdfs_per_month,
        period: 'monthly',
      },
      voice_minutes: {
        used: voiceMonth,
        limit: features?.voice_unlimited ? null : (features?.voice_minutes_per_month || 0),
        period: 'monthly',
      },
    },
  };
}

// ═══════════════════════════════════════════════════════════════════════════════
// INTERNAL HELPER FUNCTIONS
// ═══════════════════════════════════════════════════════════════════════════════

function classifyCohort(registeredAt) {
  // Alpha users: registered March 1–31, 2026
  if (registeredAt >= ALPHA_START_DATE && registeredAt < OG_CUTOFF_DATE) return 'og';
  // Early users: registered April 1–30, 2026
  if (registeredAt >= OG_CUTOFF_DATE && registeredAt < PRICING_LAUNCH_DATE) return 'early';
  // Anyone before March 1 is also treated as OG (pre-alpha)
  if (registeredAt < ALPHA_START_DATE) return 'og';
  return 'new_user';
}

function getGraceEndDate(registeredAt) {
  const cohort = classifyCohort(registeredAt);
  if (cohort === 'og') return OG_GRACE_END;
  if (cohort === 'early') return EARLY_GRACE_END;
  return null; // New users don't have a fixed grace end
}

function getDaysRemaining(graceEnd, now) {
  if (!graceEnd) return null;
  const diff = graceEnd.getTime() - now.getTime();
  if (diff <= 0) return 0;
  return Math.ceil(diff / (1000 * 60 * 60 * 24));
}

function getNextMidnightUTC() {
  const now = new Date();
  const tomorrow = new Date(now);
  tomorrow.setUTCDate(tomorrow.getUTCDate() + 1);
  tomorrow.setUTCHours(0, 0, 0, 0);
  return tomorrow;
}

async function getFreePlanFeatures(db) {
  const plan = await db.collection('subscription_plans').findOne({ id: 'free' });
  return plan?.features || {
    chat_model_tier: 'standard',
    premium_chat: false,
    standard_msgs_per_day: 50,
    images_per_month: 10,
    image_watermark: true,
    image_rate_limit_per_hour: 5,
    videos_per_month: 0,
    pdfs_per_month: 5,
    voice_chat: false,
    conversation_search: false,
  };
}

async function getStandardModels() {
  // Return list of standard models user can use instead of premium
  return [
    { id: 'gpt-4o-mini', name: 'GPT-4o Mini', provider: 'openai' },
    { id: 'gemini-2.0-flash', name: 'Gemini 2.0 Flash', provider: 'google' },
    { id: 'claude-3.5-haiku', name: 'Claude 3.5 Haiku', provider: 'anthropic' },
  ];
}

async function getStandardMsgsUsedToday(userId, db) {
  const todayStr = new Date().toISOString().split('T')[0];
  const counter = await db.collection('usage_counters').findOne({
    key: `${userId}:standard_message:daily:${todayStr}`,
  });
  return counter?.count || 0;
}

async function getPremiumMsgsUsedThisMonth(userId, db) {
  const monthStr = new Date().toISOString().substring(0, 7);
  const counter = await db.collection('usage_counters').findOne({
    key: `${userId}:premium_message:monthly:${monthStr}`,
  });
  return counter?.count || 0;
}

async function getImagesUsedThisMonth(userId, db) {
  const monthStr = new Date().toISOString().substring(0, 7);
  const counter = await db.collection('usage_counters').findOne({
    key: `${userId}:image:monthly:${monthStr}`,
  });
  return counter?.count || 0;
}

async function getImagesUsedThisHour(userId, db) {
  const hourStr = new Date().toISOString().substring(0, 13);
  const counter = await db.collection('usage_counters').findOne({
    key: `${userId}:image:hourly:${hourStr}`,
  });
  return counter?.count || 0;
}

async function getVideosUsedThisMonth(userId, db) {
  const monthStr = new Date().toISOString().substring(0, 7);
  const counter = await db.collection('usage_counters').findOne({
    key: `${userId}:video:monthly:${monthStr}`,
  });
  return counter?.count || 0;
}

async function getPdfsUsedThisMonth(userId, db) {
  const monthStr = new Date().toISOString().substring(0, 7);
  const counter = await db.collection('usage_counters').findOne({
    key: `${userId}:pdf:monthly:${monthStr}`,
  });
  return counter?.count || 0;
}

async function getVoiceMinutesUsedThisMonth(userId, db) {
  const monthStr = new Date().toISOString().substring(0, 7);
  const counter = await db.collection('usage_counters').findOne({
    key: `${userId}:voice_minute:monthly:${monthStr}`,
  });
  return counter?.count || 0;
}

/**
 * Check if a new trial user has hit any of their trial limits.
 * Trial limits are equivalent to the Base plan limits.
 */
async function checkTrialLimitsHit(userId, db) {
  const monthStr = new Date().toISOString().substring(0, 7);
  
  // Check images (50/mo trial limit)
  const images = await getImagesUsedThisMonth(userId, db);
  if (images >= BASE_TRIAL_FEATURES.images_per_month) return true;

  // Check videos (1/mo trial limit)
  const videos = await getVideosUsedThisMonth(userId, db);
  if (videos >= BASE_TRIAL_FEATURES.videos_per_month) return true;

  // Check premium messages (50/mo trial limit)
  const premiumMsgs = await getPremiumMsgsUsedThisMonth(userId, db);
  if (premiumMsgs >= BASE_TRIAL_FEATURES.premium_chat_msgs_per_month) return true;

  // Check voice (30 min/mo trial limit)
  const voice = await getVoiceMinutesUsedThisMonth(userId, db);
  if (voice >= BASE_TRIAL_FEATURES.voice_minutes_per_month) return true;

  // Check PDFs (25/mo trial limit)
  const pdfs = await getPdfsUsedThisMonth(userId, db);
  if (pdfs >= BASE_TRIAL_FEATURES.pdfs_per_month) return true;

  return false;
}

// ── Exports ─────────────────────────────────────────────────────────────────
export {
  PRICING_LAUNCH_DATE,
  OG_CUTOFF_DATE,
  OG_GRACE_END,
  EARLY_GRACE_END,
  BASE_TRIAL_FEATURES,
  UNRESTRICTED_FEATURES,
  classifyCohort,
  getGraceEndDate,
  getDaysRemaining,
};
