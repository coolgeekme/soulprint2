...
// Check status for image-to-video jobs (uses the standard jobs endpoint)
async function checkImageToVideoStatus(taskId, kieApiKey) {
  const STATUS_ENDPOINT = `https://api.kie.ai/api/v1/jobs/getTask?taskId=${taskId}`;
  ...
  // Parse the response - jobs API returns results differently
  const taskData = data.data || {};
  const status = taskData.status?.toLowerCase();
  const output = taskData.output || {};
  const videoUrl = output.video_url || output.videoUrl || taskData.video_url;
  const thumbnailUrl = output.thumbnail_url || output.thumbnailUrl || taskData.image_url;
  
  if (videoUrl) {
    return { status: 'success', videoUrl, thumbnailUrl };
  }
  if (status === 'failed' || status === 'error') {
    return { status: 'failed', error: taskData.error || taskData.message || 'Generation failed' };
  }
  return { status: 'generating', progress: status || 'generating video from image...' };
}
...
function detectVideoExtendIntent(prompt) {
  if (!prompt) return false;
  const lower = prompt.toLowerCase();
