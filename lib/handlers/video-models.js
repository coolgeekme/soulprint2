...
const VIDEO_MODEL_UX = {
  ...
  'seedance-2-0': {
    ...
    progressStages: [
      { threshold: 0, message: 'Queuing your cinematic video...' },
      { threshold: 15, message: 'Seedance 2.0 is warming up...' },
      { threshold: 30, message: 'Composing scene with native audio...' },
      { threshold: 50, message: 'Rendering frames with motion physics...' },
      { threshold: 70, message: 'Synchronizing audio & camera work...' },
      { threshold: 85, message: 'Encoding HD output...' },
      { threshold: 95, message: 'Finalizing your masterpiece...' },
    ],
  },
  'seedance-2-0-fast': {
    ...
    stuckWarningMs: 5 * 60 * 1000,
    progressStages: [
      { threshold: 0, message: 'Queuing fast video...' },
      { threshold: 20, message: 'Seedance 2.0 Fast rendering...' },
      { threshold: 45, message: 'Processing motion & audio...' },
      { threshold: 70, message: 'Encoding output...' },
  ...
  'wan-2-7': {
    ...
    progressStages: [
      { threshold: 0, message: 'Queuing your video...' },
      { threshold: 15, message: 'Wan 2.7 is warming up...' },
      { threshold: 35, message: 'Rendering multi-modality scene...' },
      { threshold: 55, message: 'Processing frames at 1080p...' },
      { threshold: 75, message: 'Enhancing visual quality...' },
      { threshold: 90, message: 'Encoding final output...' },
      { threshold: 95, message: 'Finalizing...' },
    ],
  },
  'happyhorse-1-0': {
    estimatedTimeLabel: '2-6 min',
    estimatedTimeMinSec: 120,
    estimatedTimeMaxSec: 360,
    pollTimeoutMs: 10 * 60 * 1000,
    stuckWarningMs: 7 * 60 * 1000,
    progressStages: [
      { threshold: 0, message: 'Queuing your video...' },
      { threshold: 15, message: 'HappyHorse 1.0 initializing...' },
      { threshold: 35, message: 'Generating frames with audio support...' },
      { threshold: 55, message: 'Processing multilingual content...' },
      { threshold: 75, message: 'Encoding HD output...' },
      { threshold: 90, message: 'Finalizing...' },
    ],
  },
  'hailuo-2-3': {
    ...
    progressStages: [
      { threshold: 0, message: 'Queuing your video...' },
      { threshold: 20, message: 'Hailuo 2.3 is rendering...' },
      { threshold: 40, message: 'Processing scene composition...' },
      { threshold: 60, message: 'Enhancing visual quality...' },
      { threshold: 80, message: 'Encoding final output...' },
      { threshold: 95, message: 'Finalizing...' },
    ],
  },
  'grok-imagine-video': {
    ...
    progressStages: [
      { threshold: 0, message: 'Queuing your video...' },
      { threshold: 20, message: 'Grok Imagine generating...' },
      { threshold: 45, message: 'Rendering photorealistic frames...' },
      { threshold: 70, message: 'Processing motion & detail...' },
      { threshold: 90, message: 'Encoding output...' },
      { threshold: 95, message: 'Finalizing...' },
    ],
  },
};
...
const VIDEO_MODELS = {
  ...
  'seedance-2-0': {
    ...
    label: 'Seedance 2.0',
    provider: 'ByteDance',
    description: 'Cutting-edge video with native audio sync, lip-sync dialogue, multi-shot editing, dance/motion',
    capabilities: ['text-to-video'],
    strengths: ['dance', 'music', 'dialogue', 'lip-sync', 'multi-shot', 'physical motion', 'camera control', 'audio'],
    maxDuration: 10,
    resolution: '1080p',
    apiType: 'jobs',
    generateEndpoint: 'https://api.kie.ai/api/v1/jobs/createTask',
    ...
    buildPayload: (prompt, options = {}) => ({
      ...
      input: {
        prompt: prompt,
        duration: options.duration ? String(options.duration) : 'auto',
        aspect_ratio: options.aspectRatio || 'auto',
      },
    }),
    parseCreateResponse: (data) => {
      if (data.code !== 200) throw new Error(data.msg || data.error || 'Seedance 2.0 generation failed');
      return data.data?.taskId;
    },
    parseStatusResponse: (data) => {
      if (data.code && data.code !== 200) {
        return { status: 'failed', error: data.msg || `Seedance API error (code: ${data.code})` };
      }
      ...
      if (state === 'success') {
        ...
        try {
          const resultJson = JSON.parse(data.data?.resultJson || '{}');
          videoUrl = resultJson?.video?.url || resultJson?.resultUrls?.[0] || resultJson?.url;
        } catch (e) {
          try {
            const urls = JSON.parse(data.data?.resultUrls || '[]');
            videoUrl = urls[0];
          } catch (e2) { videoUrl = data.data?.resultUrls; }
        }
        return { status: 'success', videoUrl, thumbnailUrl: data.data?.coverUrl };
      } else if (state === 'fail') {
        return { status: 'failed', error: data.data?.failMsg || 'Seedance 2.0 generation failed' };
      }
      return { status: 'generating', progress: 'processing...' };
    },
  },
  'seedance-2-0-fast': {
    ...
    provider: 'ByteDance',
    description: 'Fast Seedance 2.0 — lower latency with audio & motion quality',
    capabilities: ['text-to-video'],
    strengths: ['dance', 'music', 'dialogue', 'fast', 'quick', 'audio'],
    maxDuration: 10,
    resolution: '720p',
    apiType: 'jobs',
    generateEndpoint: 'https://api.kie.ai/api/v1/jobs/createTask',
    ...
    parseCreateResponse: (data) => {
      if (data.code !== 200) throw new Error(data.msg || data.error || 'Seedance 2.0 Fast generation failed');
      return data.data?.taskId;
    },
    parseStatusResponse: (data) => {
      if (data.code && data.code !== 200) {
        return { status: 'failed', error: data.msg || `Seedance Fast API error (code: ${data.code})` };
      }
      const state = data.data?.state;
      if (state === 'success') {
        let videoUrl = null;
        ...
        return { status: 'failed', error: data.data?.failMsg || 'Seedance 2.0 Fast generation failed' };
      }
      return { status: 'generating', progress: 'processing...' };
    },
  },
  ...
  'wan-2-7': {
    ...
    label: 'Wan 2.7',
    provider: 'Alibaba',
    description: 'Multi-modality video with 1080p output, 4K upscale, reference-to-video, video editing',
    capabilities: ['text-to-video', 'image-to-video'],
    strengths: ['multi-modality', 'editing', 'high-res', '1080p', 'reference', 'realistic', 'detailed'],
    maxDuration: 10,
    resolution: '1080p',
    apiType: 'jobs',
    generateEndpoint: 'https://api.kie.ai/api/v1/jobs/createTask',
    statusEndpoint: 'https://api.kie.ai/api/v1/jobs/recordInfo',
    buildPayload: (prompt, options = {}) => ({
      model: options.imageUrls ? 'wan/2-7-image-to-video' : 'wan/2-7-text-to-video',
      input: {
        prompt: prompt,
        ...(options.imageUrls ? { image_url: options.imageUrls[0] } : {}),
        aspect_ratio: options.aspectRatio || '16:9',
        resolution: '1080p',
      },
    }),
    parseCreateResponse: (data) => {
      if (data.code !== 200) throw new Error(data.msg || data.error || 'Wan 2.7 generation failed');
      return data.data?.taskId;
    },
    parseStatusResponse: (data) => {
      if (data.code && data.code !== 200) return { status: 'failed', error: data.msg || `Wan 2.7 API error (code: ${data.code})` };
      const state = data.data?.state?.toLowerCase() || data.data?.status?.toLowerCase();
      if (state === 'success' || state === 'completed' || state === 'succeed') {
        let videoUrl = null, thumbnailUrl = null;
        try { const r = JSON.parse(data.data?.resultJson || '{}'); videoUrl = r?.resultUrls?.[0] || r?.videoUrl || r?.video_url || r?.url; thumbnailUrl = r?.coverUrl; } catch {}
        if (!videoUrl) { const o = data.data?.output || {}; videoUrl = o.video_url || o.videoUrl; thumbnailUrl = o.cover_url || o.coverUrl; }
        if (!videoUrl) { try { const u = JSON.parse(data.data?.resultUrls || '[]'); videoUrl = u[0]; } catch { videoUrl = data.data?.resultUrls; } }
        return { status: 'success', videoUrl, thumbnailUrl: thumbnailUrl || data.data?.coverUrl };
      } else if (state === 'fail' || state === 'failed' || state === 'error') {
        return { status: 'failed', error: data.data?.failMsg || data.data?.error || 'Wan 2.7 generation failed' };
      }
      return { status: 'generating', progress: 'processing...' };
    },
  },
  ...
  'happyhorse-1-0': {
    ...
    provider: 'Alibaba',
    description: 'Emerging video model with multilingual prompts, audio generation, and reference-to-video',
    capabilities: ['text-to-video', 'image-to-video'],
    strengths: ['multilingual', 'audio', 'reference', 'creative', 'emerging', 'varied'],
    maxDuration: 10,
    resolution: '1080p',
    apiType: 'jobs',
    generateEndpoint: 'https://api.kie.ai/api/v1/jobs/createTask',
    statusEndpoint: 'https://api.kie.ai/api/v1/jobs/recordInfo',
    buildPayload: (prompt, options = {}) => ({
      model: options.imageUrls ? 'happyhorse/image-to-video' : 'happyhorse/text-to-video',
      input: {
        prompt: prompt,
        ...(options.imageUrls ? { image_url: options.imageUrls[0] } : {}),
        aspect_ratio: options.aspectRatio || '16:9',
    ...
    parseCreateResponse: (data) => {
      if (data.code !== 200) throw new Error(data.msg || data.error || 'HappyHorse generation failed');
      return data.data?.taskId;
    },
    parseStatusResponse: (data) => {
      if (data.code && data.code !== 200) return { status: 'failed', error: data.msg || `HappyHorse API error (code: ${data.code})` };
      const state = data.data?.state?.toLowerCase() || data.data?.status?.toLowerCase();
      if (state === 'success' || state === 'completed' || state === 'succeed') {
        let videoUrl = null, thumbnailUrl = null;
        try { const r = JSON.parse(data.data?.resultJson || '{}'); videoUrl = r?.resultUrls?.[0] || r?.videoUrl || r?.url; thumbnailUrl = r?.coverUrl; } catch {}
        if (!videoUrl) { try { const u = JSON.parse(data.data?.resultUrls || '[]'); videoUrl = u[0]; } catch { videoUrl = data.data?.resultUrls; } }
        return { status: 'success', videoUrl, thumbnailUrl: thumbnailUrl || data.data?.coverUrl };
      } else if (state === 'fail' || state === 'failed' || state === 'error') {
        return { status: 'failed', error: data.data?.failMsg || data.data?.error || 'HappyHorse generation failed' };
      }
      return { status: 'generating', progress: 'processing...' };
    },
  },
  ...
  'hailuo-2-3': {
    ...
    label: 'Hailuo 2.3',
    provider: 'Hailuo',
    description: 'High-quality text and image-to-video with Pro and Standard tiers',
    capabilities: ['text-to-video', 'image-to-video'],
    strengths: ['high-quality', 'cinematic', 'detailed', 'professional', 'versatile'],
    maxDuration: 10,
    resolution: '1080p',
    apiType: 'jobs',
    generateEndpoint: 'https://api.kie.ai/api/v1/jobs/createTask',
    statusEndpoint: 'https://api.kie.ai/api/v1/jobs/recordInfo',
    buildPayload: (prompt, options = {}) => ({
      model: options.imageUrls ? 'hailuo/2-3-image-to-video-pro' : 'hailuo/02-text-to-video-pro',
      input: {
        prompt: prompt,
        ...(options.imageUrls ? { image_url: options.imageUrls[0] } : {}),
        aspect_ratio: options.aspectRatio || '16:9',
    ...
    parseCreateResponse: (data) => {
      if (data.code !== 200) throw new Error(data.msg || data.error || 'Hailuo 2.3 generation failed');
      return data.data?.taskId;
    },
    parseStatusResponse: (data) => {
      if (data.code && data.code !== 200) return { status: 'failed', error: data.msg || `Hailuo API error (code: ${data.code})` };
      const state = data.data?.state?.toLowerCase() || data.data?.status?.toLowerCase();
      if (state === 'success' || state === 'completed' || state === 'succeed') {
        let videoUrl = null, thumbnailUrl = null;
        try { const r = JSON.parse(data.data?.resultJson || '{}'); videoUrl = r?.resultUrls?.[0] || r?.videoUrl || r?.url; thumbnailUrl = r?.coverUrl; } catch {}
        ...
        return { status: 'failed', error: data.data?.failMsg || data.data?.error || 'Hailuo 2.3 generation failed' };
      }
      return { status: 'generating', progress: 'processing...' };
    },
  },
  // ── NEW: Grok Imagine Video ───────────────────────────────────────────
  'grok-imagine-video': {
    id: 'grok-imagine-video',
    label: 'Grok Imagine Video',
    provider: 'xAI',
    description: 'Photorealistic video from xAI — text/image to video with upscale and extend',
    capabilities: ['text-to-video', 'image-to-video'],
    strengths: ['photorealistic', 'creative', 'upscale', 'extend', 'xai'],
    maxDuration: 10,
    resolution: '1080p',
    apiType: 'jobs',
    generateEndpoint: 'https://api.kie.ai/api/v1/jobs/createTask',
    statusEndpoint: 'https://api.kie.ai/api/v1/jobs/recordInfo',
    buildPayload: (prompt, options = {}) => ({
      model: options.imageUrls ? 'grok-imagine/image-to-video' : 'grok-imagine/text-to-video',
      input: {
        prompt: prompt,
        ...(options.imageUrls ? { image_url: options.imageUrls[0] } : {}),
        aspect_ratio: options.aspectRatio || '16:9',
    ...
    parseCreateResponse: (data) => {
      if (data.code !== 200) throw new Error(data.msg || data.error || 'Grok Imagine Video generation failed');
      return data.data?.taskId;
    },
    parseStatusResponse: (data) => {
      if (data.code && data.code !== 200) return { status: 'failed', error: data.msg || `Grok Imagine Video API error (code: ${data.code})` };
      const state = data.data?.state?.toLowerCase() || data.data?.status?.toLowerCase();
      if (state === 'success' || state === 'completed' || state === 'succeed') {
        let videoUrl = null, thumbnailUrl = null;
        try { const r = JSON.parse(data.data?.resultJson || '{}'); videoUrl = r?.resultUrls?.[0] || r?.videoUrl || r?.url; thumbnailUrl = r?.coverUrl; } catch {}
        ...
        return { status: 'failed', error: data.data?.failMsg || data.data?.error || 'Grok Imagine Video generation failed' };
      }
      return { status: 'generating', progress: 'processing...' };
    },
  },
...
// Parse explicit model request from prompt (e.g., "Use Seedance 2.0 to generate: ...")
function parseExplicitVideoModelFromPrompt(prompt) {
  if (!prompt) return null;
  
  // Model name aliases → internal model IDs
  const modelAliases = [
    { names: ['seedance 2', 'seedance 2.0', 'seedance2', 'seed dance 2', 'seeddance 2'], model: 'seedance-2-0' },
    { names: ['seedance fast', 'seedance 2 fast', 'seedance 2.0 fast', 'seedance quick'], model: 'seedance-2-0-fast' },
    { names: ['seedance', 'seed dance', 'seeddance', 'seedance 1.5', 'seedance 1'], model: 'seedance-2-0' }, // Default seedance → 2.0
    { names: ['veo 3.1', 'veo3', 'veo 3', 'google veo', 'veo'], model: 'veo3' },
    { names: ['kling 3.0', 'kling 3', 'kling3', 'kling'], model: 'kling-3.0' },
    { names: ['runway aleph', 'runway', 'aleph'], model: 'runway-aleph' },
    { names: ['sora 2 pro', 'sora pro', 'sora hd'], model: 'sora-2-pro' },
    { names: ['sora 2', 'sora2', 'sora'], model: 'sora-2' },
    { names: ['wan 2.7', 'wan2.7', 'wan 27'], model: 'wan-2-7' },
    { names: ['wan 2.6', 'wan', 'wan2'], model: 'wan-2-6' },
    { names: ['kling 2.6', 'kling 2'], model: 'kling-2-6' },
    { names: ['happyhorse', 'happy horse', 'happyhorse 1.0', 'happyhorse 1'], model: 'happyhorse-1-0' },
    { names: ['hailuo 2.3', 'hailuo2.3', 'hailuo 23'], model: 'hailuo-2-3' },
    { names: ['hailuo'], model: 'hailuo-2-3' },
    { names: ['grok video', 'grok imagine video', 'grok-imagine-video'], model: 'grok-imagine-video' },
  ];
  
  const lower = prompt.toLowerCase();
  
  // Pattern templates — each model alias gets tested against these
  for (const { names, model } of modelAliases) {
    for (const name of names) {
      const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const patterns = [
        new RegExp(`\\buse\\s+${escaped}\\b`, 'i'),
        new RegExp(`\\bwith\\s+${escaped}\\b`, 'i'),
        new RegExp(`\\busing\\s+${escaped}\\b`, 'i'),
        new RegExp(`\\bin\\s+${escaped}\\b`, 'i'),
        new RegExp(`\\bvia\\s+${escaped}\\b`, 'i'),
        new RegExp(`\\b${escaped}\\s+(?:video|generation|model)\\b`, 'i'),
        new RegExp(`(?:generate|create|make)\\s+(?:a\\s+)?(?:video\\s+)?(?:with|using|in)\\s+${escaped}\\b`, 'i'),
      ];
      
      for (const pattern of patterns) {
        if (pattern.test(lower)) {
          console.log(`[VideoModel] Explicit model requested in prompt: ${model} (matched name: "${name}")`);
          return model;
        }
      }
    }
...
3. "seedance-2-0" — Seedance 2.0 (ByteDance): Cutting-edge with native audio sync, lip-synced dialogue, multi-shot editing, director camera control (dolly zoom, tracking shots), dance/motion. Good for: dance videos, music clips, dialogue scenes, complex multi-shot narratives, physical realism.
4. "seedance-2-0-fast" — Seedance 2.0 Fast: Same as Seedance 2.0 but faster/cheaper. Good when speed is priority but still want audio + motion quality.
5. "wan-2-7" — Wan 2.7 (Alibaba): Multi-modality video with 1080p output. Excels at: detailed prompts, reference-to-video, realistic motion, multi-modal scenes.
6. "happyhorse-1-0" — HappyHorse 1.0 (Alibaba): Emerging model with multilingual audio support. Good for: creative content, varied styles, multilingual prompts.
7. "hailuo-2-3" — Hailuo 2.3: High-quality cinematic Pro video. Good for: polished professional content, detailed scenes.
8. "grok-imagine-video" — Grok Imagine Video (xAI): Photorealistic video generation. Good for: photorealistic, creative, social media content.

User request: "${prompt.substring(0, 300)}"
Has image attachment: ${hasImage ? 'yes' : 'no'}

Rules:
- Choose "seedance-2-0" for: dance, music, dialogue, lip-sync, multi-shot, physical motion, ByteDance/TikTok style, camera movements
- Choose "seedance-2-0-fast" for: same as above but user wants it fast/quick
- Choose "veo3" for: cinematic, dramatic, nature, landscape, professional, high-quality, 1080p, film-like requests
- Choose "kling-3.0" for: quick animations, character movements, image animation, general creative content, speed priority
- Choose "wan-2-7" for: detailed realistic scenes, multi-modality, reference-based generation, complex prompts
- Choose "happyhorse-1-0" for: multilingual content, creative variety, emerging styles
- Choose "hailuo-2-3" for: polished professional content, cinematic quality on par with Veo
- Choose "grok-imagine-video" for: photorealistic content, social media, xAI-style creativity
- Default to "seedance-2-0" for dance/motion, "veo3" for cinematic, "kling-3.0" for simple animations

Respond ONLY with valid JSON: {"model": "kling-3.0" or "veo3" or "seedance-2-0" or "seedance-2-0-fast" or "wan-2-7" or "happyhorse-1-0" or "hailuo-2-3" or "grok-imagine-video", "reason": "one sentence explanation"}`;
  ...
  const cinematicKeywords = ['cinematic', '1080p', 'hd', 'film', 'dramatic', 'professional', 'landscape', 'nature', 'aerial', 'sunset', 'sunrise', 'ocean', 'mountain'];
  const seedanceKeywords = ['dance', 'dancing', 'choreograph', 'lip sync', 'dialogue', 'speaking', 'talking', 'multi-shot', 'multishot', 'camera control', 'dolly', 'tracking shot', 'tiktok', 'music video', 'audio sync'];
  
  const isCinematic = cinematicKeywords.some(kw => lowerPrompt.includes(kw));
  const isSeedance = seedanceKeywords.some(kw => lowerPrompt.includes(kw));
  
  if (isSeedance) {
    return { model: 'seedance-2-0', reason: 'Seedance 2.0 selected for dance/motion/audio content' };
  }
  if (isCinematic) {
...
async function extendVideo(taskId, prompt, kieApiKey, options = {}) {
  ...
  const payload = {
    ...
    expandPrompt: true,
    quality: options.quality || '720p', // Required by Kie.ai Runway API — '720p' is safe default
    waterMark: '',
...
function detectVideoExtendIntent(prompt, hasVideoAttachment = false) {
  ...
  const extendPatterns = [
    ...
    /\b(video|clip|animation|scene)\b.*\b(extend|continue|lengthen|longer)\b/i,
