...
import { VIDEO_MODELS, VIDEO_MODEL_UX, KIE_VIDEO_MODELS, parseExplicitImageModelFromPrompt, detectAspectRatioFromPrompt, generateVideoWithModel, checkVideoStatus, checkExtendStatus, getProgressMessage, calculateModelProgress } from '@/lib/handlers/video-models';
...
function selectBestImageModel(prompt, userPreferredModel = null) {
  ...
  if (patterns.product.test(lowerPrompt)) {
    return {
      model: 'gpt-image-1-5',
      reason: '🛍️ Product/commercial - GPT Image 1.5 for clean commercial shots',
      confidence: 'high'
    };
  }
...
async function handleMediaPending(request) {
  ...
  // Find all video jobs that are still generating (last 4 hours to avoid missing slow generations)
  const pendingJobs = await db.collection('video_jobs')
    .find({ 
      user_id: user.id, 
      status: 'generating',
      created_at: { $gte: new Date(Date.now() - 4 * 60 * 60 * 1000) } // Last 4 hours
    })
    .sort({ created_at: -1 })
    .limit(20)
    .toArray();
