...
async function handleGitHubConnect(request) {
  ...
  const authUrl = `https://github.com/login/oauth/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(callbackUrl)}&scope=${encodeURIComponent(scopes)}&state=${state}`;
...
async function processGitHubChatCommand(userId, content) {
  ...
  if (!token) {
    if (/github|repo|repository|pull request|PR|commit|issue.*github|codebase/i.test(content)) {
      return {
        type: 'system_response',
  ...
  // Natural language GitHub detection — inject repo context into AI prompt
  const githubNLPatterns = [
    /\b(?:look at|check|browse|show|open|read|view)\b.*\b(?:repo|repository|codebase|github|code)\b/i,
    /\b(?:what|list|show)\b.*\b(?:issues?|pull requests?|PRs?|commits?)\b.*\b(?:in|on|for)\b.*\b(?:my|the)\b/i,
    /\b(?:help me|assist|debug|fix|review)\b.*\b(?:code|file|function|bug|error)\b.*\b(?:in|on|from)\b.*\b(?:my|the)\b.*\b(?:repo|repository|project|github)\b/i,
    /\b(?:create|open|make|submit)\b.*\b(?:issue|bug report|pull request|PR)\b/i,
    /\b(?:what changed|recent changes|commit history|last commits)\b/i,
  ];
