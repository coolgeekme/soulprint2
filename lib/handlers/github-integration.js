...
/**
 * Create a branch, commit file changes, and open a PR — all in one shot.
 * @param {string} userId - User ID to look up GitHub token
 * @param {string} owner - Repo owner
 * @param {string} repo - Repo name
 * @param {Array<{path: string, content: string}>} files - Files to create/update
 * @param {string} description - PR title/description
 * @returns {object} - { success, pr_url, pr_number, branch, error }
 */
async function createGitHubPRWithFixes(userId, owner, repo, files, description) {
  try {
    const token = await getUserGitHubToken(userId);
    if (!token) return { success: false, error: 'GitHub not connected' };

    // 1. Get the default branch and its latest commit SHA
    const { data: repoInfo } = await githubApiFetch(token, `/repos/${owner}/${repo}`);
    if (!repoInfo) return { success: false, error: 'Could not fetch repo info' };
    const defaultBranch = repoInfo.default_branch || 'main';

    const { data: refData } = await githubApiFetch(token, `/repos/${owner}/${repo}/git/ref/heads/${defaultBranch}`);
    if (!refData?.object?.sha) return { success: false, error: 'Could not get latest commit SHA' };
    const baseSha = refData.object.sha;

    // 2. Create a new branch
    const timestamp = Date.now().toString(36);
    const branchName = `soulprint/fix-${timestamp}`;
    
    const { ok: branchOk, data: branchData } = await githubApiFetch(token, `/repos/${owner}/${repo}/git/refs`, {
      method: 'POST',
      body: JSON.stringify({ ref: `refs/heads/${branchName}`, sha: baseSha }),
    });
    if (!branchOk) return { success: false, error: `Could not create branch: ${JSON.stringify(branchData)}` };

    // 3. Commit each file to the new branch
    for (const file of files) {
      // Check if file already exists to get its SHA (needed for updates)
      let fileSha = null;
      const { data: existingFile, ok: fileExists } = await githubApiFetch(token, `/repos/${owner}/${repo}/contents/${file.path}?ref=${branchName}`);
      if (fileExists && existingFile?.sha) {
        fileSha = existingFile.sha;
      }

      const commitPayload = {
        message: `fix: ${file.path} — SoulPrint AI code review`,
        content: Buffer.from(file.content).toString('base64'),
        branch: branchName,
      };
      if (fileSha) commitPayload.sha = fileSha;

      const { ok: commitOk, data: commitData } = await githubApiFetch(token, `/repos/${owner}/${repo}/contents/${file.path}`, {
        method: 'PUT',
        body: JSON.stringify(commitPayload),
      });
      if (!commitOk) {
        console.error(`[GitHub] Failed to commit ${file.path}:`, commitData);
        return { success: false, error: `Failed to commit ${file.path}` };
      }
    }

    // 4. Create Pull Request
    const prTitle = description?.split('\n')[0] || `SoulPrint AI Code Fix — ${new Date().toLocaleDateString()}`;
    const prBody = `## 🤖 SoulPrint AI Code Review Fix\n\n${description || 'Automated code fixes based on AI review.'}\n\n---\n*This PR was created by SoulPrint Engine based on an AI code review.*`;
    
    const { data: prData, ok: prOk } = await githubApiFetch(token, `/repos/${owner}/${repo}/pulls`, {
      method: 'POST',
      body: JSON.stringify({
        title: prTitle,
        body: prBody,
    ...
    if (!prOk) return { success: false, error: `Failed to create PR: ${JSON.stringify(prData)}` };
...
async function processGitHubChatCommand(userId, content) {
  ...
  if (!token) {
    // Only trigger for EXPLICIT GitHub slash commands or clear action requests
    // Don't trigger if user is just mentioning GitHub in conversation
    const isSlashCommand = /^\/github\b/i.test(content.trim());
    const isActionRequest = /\b(connect|link|auth|authorize)\s+github\b/i.test(content);
    
    if (isSlashCommand || isActionRequest) {
      return {
        type: 'system_response',
        content: '🔗 You need to connect your GitHub account first. Go to **Settings → Integrations → Connect GitHub** to get started.',
      };
    }
    // User is just mentioning GitHub in conversation - let the LLM handle it
    return null;
  }
  ...
  const githubNLPatterns = [
    ...
    /\b(?:what(?:'s| is| are))\b.*\b(?:in|on|happening|going on|open)\b.*\b(?:my|the)\b.*\b(?:repo|project|codebase)\b/i, // "what's happening in my repo"
    /\b(?:what did|show me|list)\b.*\b(?:push|commit|merge|change)\b/i, // "what did I push last week"
    /\b(?:open|any|list|show)\b.*\b(?:issues?|PRs|pull requests?|bugs?)\b/i, // "any open issues?"
  ];

  // Also detect owner/repo patterns in the message (e.g. "coolgeekme/swimqual")
  const repoMention = content.match(/\b([a-zA-Z0-9_-]+\/[a-zA-Z0-9_.-]+)\b/);

  const isGitHubNL = githubNLPatterns.some(p => p.test(content));
  // Broader action verb list for when a specific repo is mentioned
  if (isGitHubNL || (repoMention && /\b(?:review|check|analyze|audit|look|scan|debug|fix|help|read|show|browse|tell me about|what(?:'s| is)|explore|examine|inspect|summarize|explain|open|list|pull|push|commit|issue|PR)\b/i.test(content))) {
    try {
      const connection = await getUserGitHubConnection(userId);
