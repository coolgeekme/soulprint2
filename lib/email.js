...
// Determine production URL intelligently
// In production, use the production domain; in preview/dev, use NEXT_PUBLIC_BASE_URL
const getAppUrl = () => {
  // If NEXT_PUBLIC_BASE_URL contains 'preview.emergentagent', use production domain for emails
  if (process.env.NEXT_PUBLIC_BASE_URL?.includes('preview.emergentagent')) {
    return 'https://soulprintengine.ai';
  }
  // If voice-chat-enhanced.emergent.host (the deployed production), use the custom domain
  if (process.env.NEXT_PUBLIC_BASE_URL?.includes('voice-chat-enhanced.emergent.host')) {
    return 'https://soulprintengine.ai';
  }
  // Otherwise use whatever is in NEXT_PUBLIC_BASE_URL or fallback
  return process.env.NEXT_PUBLIC_BASE_URL || 'https://soulprintengine.ai';
};
...
<body>
  <div class="container">
    ...
    <div class="content">
      ...
      <center><a href="${APP_URL}/onboarding" class="btn">Get Started →</a></center>
    </div>
    <div class="footer">
...
// Alert team about model failures for monitoring/debugging
export async function sendModelFailureAlert({ userId, model, errorMessage, prompt, attemptedModels = [] }) {
  const timestamp = new Date().toISOString();
  const truncatedPrompt = prompt?.substring(0, 200) || 'N/A';
  
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #1a1f2e; color: #ffffff; border-radius: 8px;">
      <h2 style="color: #f87171; margin-bottom: 20px;">🚨 Model Failure Alert</h2>
      
      <div style="background: #0f1419; padding: 16px; border-radius: 6px; margin-bottom: 16px; border-left: 4px solid #f87171;">
        <p style="margin: 0 0 8px 0; color: #9ca3af; font-size: 12px;">TIMESTAMP</p>
        <p style="margin: 0; font-size: 14px;">${timestamp}</p>
      </div>
      
      <div style="background: #0f1419; padding: 16px; border-radius: 6px; margin-bottom: 16px;">
        <p style="margin: 0 0 8px 0; color: #9ca3af; font-size: 12px;">FAILED MODEL</p>
        <p style="margin: 0; font-size: 14px; font-weight: 600; color: #f87171;">${model}</p>
      </div>
      
      <div style="background: #0f1419; padding: 16px; border-radius: 6px; margin-bottom: 16px;">
        <p style="margin: 0 0 8px 0; color: #9ca3af; font-size: 12px;">ERROR MESSAGE</p>
        <p style="margin: 0; font-size: 13px; font-family: 'Courier New', monospace; color: #fca5a5;">${errorMessage}</p>
      </div>
      
      ${attemptedModels.length > 0 ? `
      <div style="background: #0f1419; padding: 16px; border-radius: 6px; margin-bottom: 16px;">
        <p style="margin: 0 0 8px 0; color: #9ca3af; font-size: 12px;">ATTEMPTED MODELS (FAILOVER CHAIN)</p>
        <p style="margin: 0; font-size: 13px;">${attemptedModels.join(' → ')}</p>
      </div>
      ` : ''}
      
      <div style="background: #0f1419; padding: 16px; border-radius: 6px; margin-bottom: 16px;">
        <p style="margin: 0 0 8px 0; color: #9ca3af; font-size: 12px;">USER ID</p>
        <p style="margin: 0; font-size: 13px; font-family: 'Courier New', monospace;">${userId || 'Unknown'}</p>
      </div>
      
      <div style="background: #0f1419; padding: 16px; border-radius: 6px; margin-bottom: 16px;">
        <p style="margin: 0 0 8px 0; color: #9ca3af; font-size: 12px;">PROMPT (FIRST 200 CHARS)</p>
        <p style="margin: 0; font-size: 13px; color: #d1d5db;">${truncatedPrompt}${prompt && prompt.length > 200 ? '...' : ''}</p>
      </div>
      
      <div style="margin-top: 20px; padding-top: 16px; border-top: 1px solid #374151; color: #9ca3af; font-size: 12px;">
        <p style="margin: 0;">This is an automated alert from ${APP_NAME} production monitoring.</p>
      </div>
    </div>
  `;
  ...
  return sendEmail({
    ...
    subject: `🚨 Model Failure: ${model} - ${new Date().toLocaleString()}`,
    html,
    replyTo: 'team@archeforge.com',
  });
}
