  ...
  async generateChatCompletion({ systemPrompt, messages, model, temperature = 0.7 }) {
    ...
    try {
      ...
      if (error.message?.includes('authentication') || error.message?.includes('invalid')) {
        throw new Error('Anthropic API key is invalid or expired. Please update your ANTHROPIC_API_KEY.');
      }
      throw error;
    }
  }
...
export function getProvider(providerName, model) {
  ...
  switch (resolvedProvider) {
    case 'anthropic': {
      const anthropicProvider = new ProviderAnthropic(model);
      // Fall back to OpenAI if Anthropic API key is not configured or invalid
      if (!anthropicProvider.isAvailable) {
        console.warn('[Provider] Anthropic API key not configured, falling back to OpenAI');
        return new ProviderOpenAI('gpt-4o'); // Use GPT-4o as fallback for Claude
      }
