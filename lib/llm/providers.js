...
export const WEB_SEARCH_TOOL = {
  ...
  function: {
    name: 'search_web',
    description: 'Search the internet for real-time data, current events, news, prices, or any up-to-date information.',
    parameters: {
      type: 'object',
      properties: {
...
async function buildSearchContext(userQuery) {
  try {
    ...
    return `[Real-time web search for: "${userQuery}"]\nSummary: ${result.answer}\n\nSources:\n${sources}\n[End of search results]`;
  } catch (e) {
...
class ProviderOpenAI {
  ...
  async generateStream({ systemPrompt, messages, model, temperature = 0.7, enableWebSearch = true }) {
    ...
    return { stream: this._openaiChunks(aiStream), searchMeta, didSearch };
  }
...
class ProviderAnthropic {
  ...
  async generateStream({ systemPrompt, messages, model, temperature = 0.7, enableWebSearch = true }) {
    ...
    if (enableWebSearch && finalMessages.length > 0) {
      ...
      if (userText && this._needsSearch(userText)) {
        const ctx = await buildSearchContext(userText);
        if (ctx) {
          searchMeta.push({ query: userText });
          didSearch = true;
  ...
  _needsSearch(text) {
    const keywords = ['today', 'current', 'latest', 'now', 'price', 'news', 'weather', 'stock', 'recent', '2024', '2025', 'who won', 'what happened'];
...
class ProviderPerplexity {
  ...
  async generateStream({ systemPrompt, messages, model, temperature = 0.7 }) {
    ...
    return { stream: this._perplexityChunks(aiStream), searchMeta: [], didSearch: false };
  }
...
class ProviderGemini {
  ...
  _needsSearch(text) {
    const keywords = ['today', 'current', 'latest', 'now', 'price', 'news', 'weather', 'stock', 'recent', '2024', '2025'];
    const lower = text.toLowerCase();
    return keywords.some(k => lower.includes(k));
  }
}
