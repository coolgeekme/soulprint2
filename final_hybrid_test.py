#!/usr/bin/env python3
"""
Proper HYBRID COMPOSITING test with correct flow
"""
...
def upload_base_image(token, conversation_id, car_base64):
    """Upload base image without triggering composite intent"""
    print("\n📤 Step 1: Uploading base image...")
    ...
    response = requests.post(f"{BASE_URL}/api/chat/stream",
        ...
        json={
            "content": "Here is my vehicle",  # Simple, neutral content
...
def test_hybrid_compositing(token, conversation_id, logo_base64):
    ...
    print("\n🎨 Step 2: Testing HYBRID COMPOSITING...")
...
def main():
    ...
    if upload_success and basic_compositing:
        print("\n🎉 COMPOSITING is working!")
        if hybrid_features:
        ...
        print("\n⚠️ COMPOSITING needs investigation")
        if not upload_success:
