...
import { Image, Film, MessageCircle, Check, Pencil, Sparkles, Zap, ChevronRight, Loader2, FastForward, Play, X } from 'lucide-react';

// ── Video Extension Confirmation Card ─────────────────────────────────
// Shows a preview of the source video and lets user confirm extension
export function VideoExtendConfirmCard({ refinedPrompt, sourceVideoUrl, seedImageUrl, onConfirm, onCancel, onRemoveReference, disabled }) {
    ...
    <div className="mt-3 mb-1">
      <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-3 sm:p-4">
          ...
          <div className="mb-3 rounded-lg overflow-hidden border border-white/10 relative">
            ...
            {onRemoveReference && (
              <button
                ...
                className="absolute top-2 right-2 p-1.5 rounded-full bg-black/60 border border-white/10 text-gray-400 hover:text-red-400 hover:bg-red-500/20 transition-colors"
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-medium text-gray-400 uppercase tracking-wide">Video Extension Prompt</span>
        </div>
        ...
        {isEditing ? (
          <div className="space-y-2">
            <textarea
              ...
              className="w-full bg-black/30 border border-white/10 rounded-lg p-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-blue-500/40 resize-none min-h-[80px]"
              rows={3}
              autoFocus
            ...
            <div className="flex gap-2">
              <button
                onClick={() => handleConfirm(editedPrompt.trim())}
                disabled={!editedPrompt.trim() || disabled || confirming}
                className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium transition-colors disabled:opacity-40"
              >
                {confirming ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <FastForward className="w-3.5 h-3.5" />}
                Extend Video
              </button>
              <button
          ...
          <>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                onClick={() => handleConfirm(refinedPrompt)}
                disabled={disabled || confirming}
                className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-blue-500/20 border border-blue-500/30 text-blue-300 hover:bg-blue-500/30 text-sm font-medium transition-all disabled:opacity-40"
              >
                {confirming ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <FastForward className="w-3.5 h-3.5" />}
                Extend Video
              </button>
...
// Shows the user that the system has context of their media
export function SourceMediaBanner({ imageUrl, videoUrl, onRemove }) {
  if (!imageUrl && !videoUrl) return null;
  
  return (
    <div className="mt-2 mb-1 flex items-center justify-between gap-2 px-3 py-2 rounded-lg bg-white/[0.03] border border-white/[0.06]">
      {videoUrl && (
        <div className="flex items-center gap-2 text-xs text-blue-400">
          <div className="w-10 h-7 rounded overflow-hidden border border-white/10 bg-black flex-shrink-0">
            <video src={videoUrl} className="w-full h-full object-cover" muted playsInline />
          </div>
          <span className="text-gray-500">Using your video as reference</span>
        </div>
      )}
      {imageUrl && !videoUrl && (
        <div className="flex items-center gap-2 text-xs text-purple-400">
          <div className="w-7 h-7 rounded overflow-hidden border border-white/10 bg-black flex-shrink-0">
            <img src={imageUrl} alt="" className="w-full h-full object-cover" />
          </div>
          <span className="text-gray-500">Using your image as reference</span>
        </div>
      )}
      {onRemove && (
        <button
          onClick={onRemove}
          className="flex-shrink-0 p-1 rounded-md hover:bg-white/10 text-gray-600 hover:text-red-400 transition-colors"
          title="Remove reference"
        >
          <X className="w-3.5 h-3.5" />
        </button>
